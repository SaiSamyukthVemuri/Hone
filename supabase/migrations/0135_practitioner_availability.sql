...
-- Swap unique(studio_id, day_of_week) for two partial uniques: at most one
-- studio-wide row per weekday, and at most one row per (practitioner, weekday).
alter table public.studio_availability_default
...
create unique index studio_availability_default_studiowide_uidx
  on public.studio_availability_default (studio_id, day_of_week)
  where practitioner_id is null;
drop index if exists studio_availability_default_perpractitioner_uidx;
create unique index studio_availability_default_perpractitioner_uidx
  on public.studio_availability_default (studio_id, day_of_week, practitioner_id)
...
create unique index studio_availability_overrides_studiowide_uidx
...
create unique index studio_availability_overrides_perpractitioner_uidx
  on public.studio_availability_overrides (studio_id, effective_date, practitioner_id)
  where practitioner_id is not null;
