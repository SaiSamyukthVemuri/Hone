...
--     - updates the rule, deletes its future occurrences, then
--       re-materializes through the horizon. All within one
--       transaction so a conflict on the new schedule leaves the
--       old schedule untouched.
--
--   delete_recurring_break_rule(p_rule_id)
--     - removes future occurrences and their shadow rows, then
--       deletes the rule. The FK on occurrences.rule_id uses
--       ON DELETE SET NULL so past-history occurrence rows survive
--       as orphan records (their shadow rows preserve audit of
--       the protected time).
--
-- Owner UI is the only privileged caller; server actions run the
-- RPCs through the authenticated session client. RLS does NOT allow
-- direct writes from any client: writes flow through the RPCs.
-- The mirror trigger is SECURITY DEFINER and bypasses RLS for
-- shadow upserts.
--
-- Install order:
...
create policy "studio_recurring_break_rules_owner_all"
  on public.studio_recurring_break_rules
  for all
...
begin
  ...
  -- Future occurrences only; the AFTER DELETE trigger removes
  -- shadow rows for the deleted occurrences.
  delete from public.studio_recurring_break_occurrences
  where rule_id = p_rule_id
    and occurrence_date >= v_today_loc;
...
revoke execute on function public.update_recurring_break_rule_and_rematerialize(
  uuid, text, integer[], time, time, boolean, date
) from public;
revoke execute on function public.update_recurring_break_rule_and_rematerialize(
  uuid, text, integer[], time, time, boolean, date
) from anon;
...
revoke execute on function public.delete_recurring_break_rule(uuid) from public;
revoke execute on function public.delete_recurring_break_rule(uuid) from anon;
revoke execute on function public.delete_recurring_break_rule(uuid) from authenticated;
grant execute on function public.delete_recurring_break_rule(uuid) to service_role;
