...
**Status: implemented in PR #241; MERGED INTO THE TODAY CARD (2026-07-31), still rules-based only.** No AI, no model call, no provider integration, no chatbot, no autonomous action. The separate "Daily prep brief" card is RETIRED: it re-rendered every one of today's appointments a second time on the same screen, in a different order, and disagreed with the Today card about the same facts (the caution printed under "Remember:" up top and again under "Caution noted:" below). The derivation survives under a combined name — the pure helper `lib/dashboard/today-workflow.ts` turns the same facts the Dashboard already loads (today's appointments, the Before Today preview pipeline, the linked-session charting state, and intake status) into ONE deterministic item per appointment, keyed by appointment id, rendered inside that appointment's Today card. Priority is retained as an in-card attention signal only and no longer reorders the day. It reads no sensitive surface and links only to existing routes; nothing is sent, charged, or mutated. The combination added no database query. Missing Records / Follow-up Assistant V1 below has since shipped (PR #249); only the Draft-only Client Message Assistant remains a future PR.
