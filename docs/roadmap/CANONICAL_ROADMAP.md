...
| Source and hosted observations | **VERIFIED_REPO:** production branch is `a946a983ac9b8da379bc869e21b32a5a3d50e548` after merged #712. **VERIFIED_PROVIDER:** the Vercel production deployment serving `hone.care` / `www.hone.care` carries that exact 40-character SHA. **VERIFIED_REPO:** hosted migration max is `0198` and repository max is `0198` **as DECLARED by the canonical record**. ⚠️ **CORRECTED 2026-09-19:** the clause *“`0199` is unallocated”* is **false and dangerous to act on** — `0199` was **applied to production on 2026-09-18** from the unmerged #716 head, so production’s OBSERVED max is `0199` while the production tree lacks the file. **The next number an author may claim is `0200`.** See `docs/production/migration-state.json` → `$hosted_discrepancy`. #709 and its 0197 are merged and applied. [§3; S37–S40, S42] |
