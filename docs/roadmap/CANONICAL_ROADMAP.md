...
| Source and hosted observations | **VERIFIED_REPO:** production branch is `a946a983ac9b8da379bc869e21b32a5a3d50e548` after merged #712. **VERIFIED_PROVIDER:** the Vercel production deployment serving `hone.care` / `www.hone.care` carries that exact 40-character SHA. **VERIFIED_REPO:** ⚠️ **CORRECTED AND THEN RECONCILED 2026-09-19.** This row read *“hosted migration max is `0198` and repository max is `0198`; `0199` is unallocated”*. `0199` was in fact **applied to production on 2026-09-18** from the reviewed #716 head, leaving production ahead of a repository that did not contain the file. **That divergence is now closed**: the applied `0199` file and its record were carried into the tree, so hosted max is **`0199`**, repository max is **`0199`**, they are at **parity** with nothing pending, and **the next free number is `0200`**. `0199` is APPLIED and FROZEN — never edit it; any correction is a new forward migration. #709 and its 0197 are merged and applied. [§3; S37–S40, S42] |
...
WAIT-03 core is **MERGED / DEPLOYED** and, since v1.15, **PRODUCTION-EXERCISED END TO END** on the controlled test studio. The Invitation-capacity gap that v1.15 recorded — `no_admission_round` — was repaired by #709, which merged on 16 September with its migration 0197; #713 then added the live-invitation read as 0198. The post-0198 canary **passed all twelve seams** on the test studio, covering capacity opening, invitation, recipient proof, scoped slot offers, booking, atomic waitlist conversion and the fail-closed refusal on exhausted capacity. Post-canary cleanup is complete and no open canary state remains. #712 then shipped **WAIT-04A**, wiring three commands migration 0193 had already applied, and is the current production head. Willow’s existing waitlist is preserved. **CORRECTED 2026-09-19 (WAIT-DOCS-RECON) — recorded, not deleted:** this sentence read *“Willow durable WAIT remains deliberately OFF and no durable cutover has been performed”*, and that was **false when written**. Willow has been on the **durable** commit point since on or before **2026-08-25**; a read-only query on 2026-09-19 returned **28 durable Willow rows**, 27 of them `waiting`. The cutover was performed without a release record. What has **not** happened on that studio is anything beyond joining: **zero invitations have ever been issued there**, and owner device acceptance is outstanding. The continuity defect Chloe raised — rollout gating hiding the Waitlist navigation while the list still exists — is unchanged and still owed. WAIT-04 seams S1–S5 and Chloe device acceptance remain required; **no provider or SMS activation has occurred.** [§§3, 14.5, 15, 17, 23; S37–S40, S42]
...
both are now applied; `0199` followed on 2026-09-18 and is applied too, leaving `0200` unallocated. v1.15 recorded `no_admission_round`
...
| --- | --- | --- |
| Production source | ⚠️ **SUPERSEDED OBSERVATION, PRESERVED AS DATED HISTORY (noted 2026-09-19).** What follows was true at the 17 September sync and is **not** the current head: production has advanced at least through #715, #727, #730, #734, #736 and #738. A fresh `git rev-parse origin/claude/build-hone-saas-hOex7` on 2026-09-19 returned **`11f2e9be121c6d02a9b1d29ee781b26bfb9c4b6b`** (merge of #736). Re-read it rather than quoting either SHA. THE DATED OBSERVATION: **VERIFIED_REPO:** protected `claude/build-hone-saas-hOex7` at `a946a983ac9b8da379bc869e21b32a5a3d50e548`, merge of #712 (WAIT-04A). Its two parents are `77d092b5…` (the #713 merge) and `f690825b…` (the #712 head), each confirmed as an ancestor. | A repository SHA alone is not serving proof; the deployment row binds the live aliases separately. [S42] |
...
| Hosted DB / migration ownership | ⚠️ **CORRECTED 2026-09-19 — THIS ROW CONTRADICTED ITS OWN CITED AUTHORITY.** It read: *“hosted max is `0198` per `docs/production/migration-state.json` … **`0199` is not allocated and not applied**”*. `0199` **WAS** applied, on **2026-09-18**, from the reviewed #716 head. **VERIFIED_REPO:** hosted max is now **`0199`** per `docs/production/migration-state.json`, repository max is **`0199`** by census of `supabase/migrations/*.sql`, and apply records for 0197, 0198 and 0199 are in `docs/production/migration-ledger.md`. | 🛑 **DO NOT CLAIM `0199` — IT IS APPLIED AND FROZEN.** The next free number is **`0200`**, claimable only by a new single-allocator decision. Parity holds: nothing sits above the hosted max and nothing is pending. Hosted state is declared in `migration-state.json`, never derived from filenames. [S42] |
...
| HOLD — numbering | **SUPERSEDED 2026-09-17 — recorded, not deleted.** The hold read: "0197 is allocated to #709 and remains pending; hosted max is 0196. Treat 0198 as next-free only after 0197 apply/verify/reconcile releases the migration lock." Both 0197 and 0198 are now applied, `0199` followed on 2026-09-18, and hosted max is **`0199`**. The next free number is **`0200`, unallocated**. WAIT-04, MPX and FIN still do not claim a number from this document; S1–S5 migrations require a new single-allocator decision. |
...
Migration truth is now: hosted/applied 0192–**0199**, with parity between hosted and repository maxima and nothing pending; next-free is **`0200`**, unallocated. The 0194 schema is applied through the WAIT assembly while #674 runtime routing remains a separate, unperformed provider operation. New WAIT-04, MPX-02 and FIN migrations remain unnumbered. The decision index (§0.4) is a pointer, not a second ledger.
