...
| Source and hosted observations | **VERIFIED_REPO:** production branch is `a946a983ac9b8da379bc869e21b32a5a3d50e548` after merged #712. **VERIFIED_PROVIDER:** the Vercel production deployment serving `hone.care` / `www.hone.care` carries that exact 40-character SHA. **VERIFIED_REPO:** hosted migration max is `0198` and repository max is `0198`; `0199` is unallocated. #709 and its 0197 are merged and applied. [§3; S37–S41] |
...
| --- | --- |
| Sam + WAIT-04 seam owner | **#709 is closed — merged 16 Sep, 0197 applied, and the P2 migration-state record resolved.** The standing role is now the WAIT-04 seam sequence, starting with its zero-migration units: the S2 invalidate caller, the S3 resolver caller with a fail-closed assertion, and the S1 preference-grant callers. Do not reopen shipped 0197/0198 runtime without a new finding. |
| Sam / release operator | Run read-only hosted preflight for 0197; with a fresh explicit T3 GO, apply/verify/reconcile 0197 before merging the application revision. Production concurrency remains one. |
...
| Sam + Chloe | Restore trustworthy waitlist discoverability, implement owner-controlled New client intake, reconcile Willow’s existing queue and only then authorize Willow durable cutover. Complete WAIT-04 and record Chloe device acceptance before declaring Willow WAIT launch DONE. |

**Publish once; maintain one set of decisions**

The matching Markdown file is prepared for reviewed synchronization to docs/roadmap/CANONICAL_ROADMAP.md. It has not been committed. After synchronization, that file is the maintained working edition; the brief and DOCX are derived publications. Update on a material decision or evidence change—not every commit. [§0.5]
...
Publication status for v1.15: this DOCX and matching Markdown edition are derived from the attached v1.14 pair (hashes in Appendix D) plus fresh GitHub evidence and Sam’s production/canary observations from 15 September local / 16 September UTC. This document update itself performs no repository write, merge, hosted migration, provider action or customer send. Repository synchronization remains PENDING. Until reviewed sync lands, identify this working copy as v1.15; an older repository copy must not silently override the newer accepted decisions. [S37–S41]
...
| Merged / deployed since v1.15 | #709 Invitation-capacity repair (migration 0197, merged 16 Sep); #713 owner read of `declined_at` completing the live predicate (migration 0198, merged 16 Sep); #712 WAIT-04A command wiring, no migration, merged 17 Sep. Production branch is now `a946a983…`. [S41] |
| Hosted / production exercised | 0192–0196 applied at the v1.15 checkpoint; **0197 and 0198 have since been applied and recorded**, leaving hosted max `0198`. The post-0198 controlled canary ran the full twelve-seam journey to **CANARY_PASS**, including booking and atomic conversion, and post-canary cleanup is complete. No provider/SMS activation and no real message were part of any of it. [S41] |
| Active release candidate | None. The v1.15 candidate #709 **merged** on 16 September and its 0197 is applied; the P2 current-state migration-record contradiction it carried is resolved — `docs/production/migration-ledger.md` records both the 0197 and the 0198 apply. [S41] |
| Still required for Willow WAIT launch | The #709/0197 and canary steps are **done**. What remains: restore Willow waitlist continuity → owner-controlled intake mode → reconcile/migrate Willow → complete WAIT-04 seams S1–S5 (S1 recipient responses 2 of 4; S2 proof credential shape; S3 studio delivery identity, provider proof outstanding; S4 reminder/48h/SMS/STOP; S5 cancellation and re-entry) → Chloe device acceptance. [§§14.5, 15, 17, 23; S41] |
...
both are now applied and `0199` is unallocated. v1.15 recorded `no_admission_round`
...
| --- |
| v1.16 refreshes only the named facts below. Repository and hosted facts are fresh reads of their own authorities; the canary, cleanup and deployment facts are the durable release records of 16–17 September. §3.1–§3.7 retain historical provenance from v1.14 and §3.8 records the v1.15 checkpoint this edition supersedes — none of them is the current release state. [S37–S41] |

| **Domain** | **Current evidence** | **Boundary / next verification** |
| --- | --- | --- |
| Production source | **VERIFIED_REPO:** protected `claude/build-hone-saas-hOex7` at `a946a983ac9b8da379bc869e21b32a5a3d50e548`, merge of #712 (WAIT-04A). Its two parents are `77d092b5…` (the #713 merge) and `f690825b…` (the #712 head), each confirmed as an ancestor. | A repository SHA alone is not serving proof; the deployment row binds the live aliases separately. [S41] |
| Serving deployment | **VERIFIED_PROVIDER:** Vercel deployment `dpl_H1ZcHgPARRR8csuN9yRscgiqY7fZ` READY, target production, `meta.githubCommitSha` = `a946a983…` — exact 40-character equality with the production head. Aliases `hone.care` and `www.hone.care` resolve to it. Resolved **by hostname**, so the answer is the deployment the domain actually serves. | Revalidate the deployment SHA at the next release boundary. No claim of authenticated browser acceptance beyond the recorded canary. [S41] |
| Hosted DB / migration ownership | **VERIFIED_REPO:** hosted max is `0198` per `docs/production/migration-state.json`, with apply records for both 0197 and 0198 in `docs/production/migration-ledger.md`. Repository max is `0198` by census of `supabase/migrations/*.sql` (197 files). **`0199` is not allocated and not applied.** | Parity holds: no migration sits above the hosted max and none is pending. The next number is `0199`, claimable only by a new single-allocator decision. Hosted state is declared in `migration-state.json`, never derived from filenames. [S41] |
| WAIT-03 runtime | #708 core is MERGED / DEPLOYED, repaired by #709 (0197) and completed by #713 (0198). WAIT-04A shipped in #712 with no migration. The test studio is enabled for legacy gate + durable WAIT. | Production-exercised end to end on the **test studio only**. A twelve-seam controlled pass is not Willow acceptance and not Chloe device acceptance. [S41] |
| Controlled canary | **CANARY_PASS — 12 of 12 seams** on the post-0198 head: Waiting, capacity opened, capacity re-read after reload, invite, translated practitioner outcome, delivery result, recipient proof, scoped eligible slots only, booking, atomic waitlist conversion, reload truth, and fail-closed refusal on exhausted capacity. **POST_CANARY_CLEANUP = COMPLETE**; no residual open canary state. The converted entry, redeemed invitation and resulting appointment were deliberately left in place — the database records that the canary happened. | The canary ran on the controlled test studio with a synthetic identity. It is evidence of the journey working, not of customer acceptance, and it involved no provider send. [S41] |
...
| Willow continuity | Existing Willow waitlist remains enabled/preserved on the legacy gate; Willow durable WAIT is deliberately OFF. Chloe observed that the Waitlist navigation can disappear when the durable flag is off, despite the list/data still existing. | Restore truthful discoverability before Willow migration. Do not solve the navigation defect by silently durable-enabling Willow. [S37] |
...
| New-client waitlist / admission control | WAIT-03 is merged/deployed through #708, repaired by #709 (0197) and completed by #713 (0198); hosted 0192–0198. The controlled test studio has production-exercised the **full twelve-seam journey**, including invitation, booking and atomic conversion. WAIT-04A shipped in #712. Willow remains on the legacy gate with durable WAIT OFF until continuity, owner control and reconciliation are complete. WAIT-04 seams S1–S5 remain accepted/unverified follow-on policy, and nine commands applied to production still have no caller. [§§3, 14.5, 15; S41] |
...
| WAIT-ENTRY-01 | Manual/legacy #685 + profile WAIT-04A,B | Durable entry works for the controlled join. Legacy/profile-completion and original-priority migration remain before Willow durable cutover. |
...
| MIGRATE WILLOW | **Continuity + owner setting + queue reconciliation** | Reconcile Chloe’s existing legacy/email-only people, preserve original priority/provenance, show Chloe the resulting list, then explicitly enable durable WAIT. Never use navigation disappearance as migration. |
...
| HOLD — release / activation | Real provider sends, Willow durable enablement/migration and WAIT-04 effective policy each require their exact current gates and separate human authorization. The 0197 apply and #709 merge this row previously held are **done**; the hold now covers what follows them. |
| HOLD — numbering | **SUPERSEDED 2026-09-17 — recorded, not deleted.** The hold read: "0197 is allocated to #709 and remains pending; hosted max is 0196. Treat 0198 as next-free only after 0197 apply/verify/reconcile releases the migration lock." Both 0197 and 0198 are now applied and hosted max is `0198`. The next free number is **`0199`, unallocated**. WAIT-04, MPX and FIN still do not claim a number from this document; S1–S5 migrations require a new single-allocator decision. |
...
Migrations **0192–0198 are recorded hosted/applied**; never edit any of them. 0197 (the Invitation-capacity consumed-count gateway, #709) and 0198 (the live-invitation read, #713) completed the WAIT-03 chain. Hosted max is `0198`, repository max is `0198`, and the next free number is **`0199` — unallocated, and claimable only by fresh census plus a single-allocator decision, never by assumption.** The 0194 schema extracted from the communications work is applied, but #674 runtime sender routing/cutover remains a separate provider operation that no one has performed; schema presence is not provider readiness. Additional WAIT-04/MPX/FIN migrations require a new single-allocator decision. [S41]
...
| **#** | **Item** | **IDs** | **State / gate** |
| --- | --- | --- | --- |
| 1 | **Willow WAIT launch — finish the production-exercised core, owner control and accepted policy** | ~~#709/0197~~ done; ~~test-studio canary~~ done; WAIT-CONTINUITY-01; WAIT-SAAS-01; Willow reconciliation/migration; WAIT-04A done, B/C/D open | **PRIMARY — rank unchanged.** §23.1 is the exact sequence. #708/0192–0198 plus WAIT-04A is shipped core and a passed twelve-seam canary, not launch completion. Remaining: trustworthy owner control, then Willow cutover and the frozen §14.5.7/§14.5.9 contract with Chloe acceptance. |
...
Migration truth is now: hosted/applied 0192–0198, with parity between hosted and repository maxima and nothing pending; next-free is `0199`, unallocated. The 0194 schema is applied through the WAIT assembly while #674 runtime routing remains a separate, unperformed provider operation. New WAIT-04, MPX-02 and FIN migrations remain unnumbered. The decision index (§0.4) is a pointer, not a second ledger.
...
### S41 · Post-#712 production evidence, reverified at synchronization time
