...
- Repository/hosted migration max: **`0165`** (both — repo == hosted) (both — repository and hosted migration state match), as of 2026-08-02. `0163` (intake INSERT boundary) was **APPLIED to production 2026-08-02T17:37Z** and is **frozen**, as is `0162` (intake review transition integrity, applied earlier the same day); the next migration number is **`0164`**. `0158` is deliberately skipped and will never be applied; `0159` (signed-record retirement) and `0160` (immutable clinical lineage) are both applied and verified. `0161` (service order RPC + widened calendar-colour CHECK) was **APPLIED to production 2026-07-30T23:38:07Z→23:38:16Z** and independently verified **as of that date**: hosted max `0161` exactly once and no `0162` yet, the services checksum UNCHANGED (zero business rows rewritten), both RPCs SECURITY DEFINER with a pinned search_path and revoked from `anon`, and the colour CHECK VALIDATED with the ten allowed keys. *(This line previously read "expected: `0133`; no `0134`", then `0157`, then `0159`, then `0161` — all superseded.)*
