...
- **FK ON DELETE rules**: `studio_id` CASCADE (matches manual_fee + studio_payment_settings + client_payment_methods); composite client + appointment + created_by_practitioner RESTRICT (matches manual_fee); `session_id` SET NULL (matches `sessions.appointment_id` from migration 0068 because sessions are immutable clinical artefacts that must survive parent-row removal). All financial-audit FK pointers preserve their target rows; no charge attempt is ever orphaned.
