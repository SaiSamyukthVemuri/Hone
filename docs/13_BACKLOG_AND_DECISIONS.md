...
### Pilot control sheet automation (PR #160)

**Decision:** Add `pilot-control/` with five human-editable YAML trackers (`chloe-testing-queue`, `product-feedback`, `launch-blockers`, `pr-build-log`, `future-ideas`). A node ESM script `scripts/export-pilot-control-sheet.mjs` validates required fields per tracker, writes deterministic CSVs to `pilot-control/generated/`, and writes an operator-facing `Hone_Pilot_Control_Sheet.xlsx` at the repo root. `npm run pilot:export` regenerates both; `npm run pilot:check` (wired into GitHub Actions CI and the `npm run ci` composite) fails when YAML and CSV are out of sync, a required field is missing, or the YAML is syntactically invalid. The XLSX is gitignored; the CSVs are the canonical CI-checked artifact. The PR template carries a "Chloe / Pilot Control Sheet" checklist with five boxes so every future PR updates the tracker if it affects Chloe / client testing.

**Why YAML + CSV instead of a SQLite or a Notion API:** YAML is human-editable in the same way Markdown is; Chloe, Claude, and ChatGPT can all edit it without tooling. CSV is universally importable and trivially diffable in PR review. A database would require a separate runtime and a sync layer; a Notion API integration would require credentials, auth, and rate-limit handling. The local file path is the cheapest first cut that satisfies the operator-facing requirement.

**Why a custom XLSX writer instead of `exceljs`:** `exceljs` is a sizable dependency for what is ultimately a flat-data export with no formatting. The script assembles a minimal OOXML workbook with `jszip` (already a dependency for Stripe Connect) in ~150 lines. Deterministic mtime + shared strings + one sheet per CSV. No styles, no formulas, no macros. The operator gets a working Excel file; a future PR can switch to `exceljs` if real styling (frozen rows, autofilter, colors) becomes a hard ask.

**Why the XLSX is NOT checked in:** binary diffs are hard to review. The CSVs carry the same data in a plain-text shape and are byte-stable across runs; CI checks those. The XLSX is regenerated on demand by anyone who needs it (`npm run pilot:export`).

**Honest non-claims:** this PR does not touch app runtime behavior. No schema change, no migration, no payment behavior change, no Stripe gate change, no portal change, no webhook change, no SMS/email change. No Notion/Google Sheets API. No background job. The pilot tracker is for operator coordination only; the application reads none of this data.
