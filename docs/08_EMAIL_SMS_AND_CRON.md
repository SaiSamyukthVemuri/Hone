...
- `reminder_scheduler_degraded` (warning) / `reminder_scheduler_stale` (**critical**) / `reminder_scheduler_missing` (critical) — **PR #283, hardened by PR OPS-01**: recorded by the best-effort scheduler-health check (`recordReminderSchedulerHealthAlert`, `lib/cron/reminder-heartbeat.ts`), which now runs from **three** independent daily Vercel crons. Deduped on an existing unresolved `ops_alerts` row for the same event, so three daily callers still record at most **one** alert per outage per event. `safe_details` carry only `status`, `last_success_at`, `age_minutes`, `previous_success_at`, `observed_interval_minutes`, `cadence_evidence`, `cadence_minutes`, `degraded_after_minutes`, `stale_after_minutes`, `checked_at` — never `CRON_SECRET`, an Authorization header, client phone/email/PII, reminder contents, or a provider payload. The check NEVER sends reminders and never calls `/api/cron/appointment-reminders`.
...
**Alert + dedupe behavior.** Each unhealthy state records **one** deduped ops alert; while an unresolved alert **of equal or higher severity** for that event exists, later checks record **nothing** (no spam). The three states are **separate events**, so a worsening outage still escalates: an unresolved `degraded` warning does not suppress the `stale` critical. **Dedupe is severity-aware (OPS-01.1).** Matching on event + unresolved alone was only safe while an event had one severity forever; OPS-01 changed `reminder_scheduler_stale` from warning to critical, so a leftover warning-severity row would otherwise have swallowed the first real critical escalation — and the `OPS_ALERT_EMAILS` notification with it. A lower-severity open row never suppresses a higher-severity alert, and the legacy warning is **not** auto-resolved: the operator resolves both rows from the same admin page.
