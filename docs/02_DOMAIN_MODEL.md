...
- **Authoritative structured areas (`session_block_areas`, migration 0128; contained by 0158).** A block may treat several areas with one set of machine settings, each with its own laterality, so the multi-area child rows — not the legacy `primary_area`/`side` pair above — are the authoritative record. The read contract lives in `lib/sessions/block-areas.ts`: **if a block has `session_block_areas` rows they win**; the legacy projection is only the fallback for blocks that have none (which is why every write also persists a *safe* legacy projection — first area, and a block-level `side` only when every area shares one). **Migration 0158 changes five facts about this table** (migration-first; not applied at the time of writing — production max is 0157): browser roles lose all DML and keep only studio-scoped `SELECT`; the 0128 `FOR ALL` policy becomes the SELECT-only `session_block_areas_member_select`; a `session_block_areas_guard_finalized` row trigger freezes the rows — insert, edit, reorder, reassign and delete — once the parent encounter is finalized, void, soft-deleted **or has ever been signed**, for **every** application-reachable role including `service_role`; a companion `session_blocks_guard_signed_write` trigger closes the two routes that change a signed record's areas *without* writing this table at all (deleting the parent block, and reparenting a block into or out of the record); the 0128 studio-derive trigger is widened to re-derive `studio_id` on an UPDATE of that column; and the only remaining write path is the trusted `SECURITY DEFINER` RPCs (`create_session_block_with_areas` / `update_session_block_with_areas`, 0129, plus `copy_session_setup`, 0157). **Caveat, stated plainly:** 0158 freezes these rows but does **not** put them in the signed clinical snapshot and does **not** make them correctable — a finalized record's authoritative areas are not yet covered by its `content_hash`, and a mis-recorded area cannot be corrected into a new signed version. That is snapshot v2, a mandatory prerequisite for enabling clinical finalization on any studio — see [docs/runbooks/0158-finalized-structured-area-containment.md](./runbooks/0158-finalized-structured-area-containment.md).
...
- **PR #190 (migration 0082), clinical memory moat.** Every session block can carry a structured client response: `tolerance_rating` (a 1-5 smallint, 1 = least comfortable to 5 = most), `reaction_type` (allowlisted vocabulary: none / mild redness / moderate redness / swelling / sensitivity / irritation / other), `reaction_notes`, `caution_for_next_session` + `caution_note`. Captured in the one-page charting form's optional "Client response" section; all fields nullable so historical blocks render unchanged. Sessions carry `next_session_note`: the plan for the NEXT visit, written while charting this one ("Plan for next visit" card on the session page).
- **PR #279 (migration 0095), Chloe charting feedback.** Two more nullable/defaulted block facts: `numbing_status` (text; NULL = "Not recorded", else `none` = "No numbing used" / `used` = "Numbing used" — a factual record, no advice/dosing) and `probe_lot_confirmed` (boolean, default false — whether the practitioner explicitly confirmed the probe lot/batch for this treatment; the lot itself may be auto-suggested from `record_keeping_sterile_items` but is never saved as confirmed without the tap). Same PR: **client tolerance is now a label-based control** (Comfortable … Needed pause/stopped early) over the **unchanged** `tolerance_rating` 1-5 storage (`lib/sessions/clinical-response.ts` is the label source); energy level moved under Treatment readings (UI only); OmniBlend charts galvanic before thermolysis with no thermolysis duration / galvanic intensity; observation chips are toggles. Legacy blocks render unchanged (numbing NULL, not confirmed).
- **PR #280 (migration 0096), Chloe record-keeping feedback.** `record_keeping_disinfectants` gains `discard_due_date` (date, nullable) — the "replace by" date a prepared batch must be discarded by, deliberately **distinct** from `date_prepared` (made) and `date_discarded` (actually discarded). The Record Keeping page renders a read-time **Due today / Overdue / Due soon** badge from it in the studio timezone (`lib/record-keeping/disinfectant-status.ts`; no cron/notification). Same PR (no schema change for these): the disinfectant **operator** is a same-studio staff dropdown (current user + active practitioners) writing the pre-existing `operator_practitioner_id` + `operator_name`, with a free-text "Other" fallback; and the **exposure-incident** form gains a Client / Staff / Other person selector that autofills the existing free-text `exposed_person_*` fields from same-studio clients/staff (editable; no client/practitioner FK stored, so the 0088 owner-only read posture is unchanged). Legacy disinfectant/exposure rows render unchanged.
...
- **Clinical lineage enforcement (PR #286, no migration).** Charting mutations enforce the full `studio → client → session → block → entry` lineage. The charting actions (`block-actions.ts`) validate that the session belongs to the **route client** (not just the studio) via the shared `assertSessionForClient` (`lib/sessions/session-lineage.ts`) before any block/entry write; every block/entry write is then scoped by that client-validated `session_id`, and migration 0094 guarantees block∈session / entry∈block∈session — so a same-studio **wrong-client** write (Client A route + Client B's session/block/entry id, e.g. from a stale tab) is rejected with a generic "Treatment session not found." (`actions.ts` already enforced this via `assertSessionVisible`.) Defense-in-depth above RLS; no schema/charting-behavior change.
...
- **No public URLs.** Viewing mints a short-TTL signed URL via a server action that re-checks the row's studio; upload + signing use the service-role client after that ownership check. Server-generated path `<studio_id>/<client_id>/<uuid>.<ext>`. Practitioner UI at `/clients/<id>/images`, surfaced as the **"Treatment Photos"** tab-bar link (PR #272 polish: styled upload card + gallery grid; PR #273: inline previews — the page server-pre-signs a short-TTL preview URL per image and the grid renders inline `<img>` + an in-app modal; signed URLs stay short-TTL, never public, never stored in the DB; PR #274: each card + the larger preview show display-only **context tags** — scope ("Client/Session/Block photo") + "Treatment area: …"/"Area not recorded" — derived from the existing `session_id`/`session_block_id` links + the embedded `session_blocks` area fields, computed server-side with no raw IDs/paths shown). Not reused: the legacy unwired `photos` table (0001). No annotation/OCR/AI/public exposure/side-by-side comparison/thumbnail pipeline (deferred). **Attaching a photo to a session and/or session-block at upload time is now live (PR #284, no migration):** the Treatment Photos upload form has a "Photo context" selector (Client photo / Session photo / Treatment area photo) that populates the existing nullable `session_id` / `session_block_id` columns. The upload action validates every submitted id **server-side** (session ∈ studio+client; block ∈ its session + studio, with the session derived from the block) before storing — mirroring the 0093 parent-consistency trigger (the structural backstop), so a cross-studio/cross-client/mismatched id is rejected with a generic error and never stored. The PR #274 context tags then render automatically for the populated rows. Storage/security is unchanged (private bucket, service-mediated, sanitizer + EXIF strip, no public URLs).
