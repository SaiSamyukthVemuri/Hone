...
- **[CORRECTED 2026-07-27]** ~~"Google Calendar sync exists." (Currently false. Read-only ICS feed only.)~~ — too blunt to be useful now. The accurate rule: **do not say Google Calendar is syncing, enabled or active** (it is dormant — Willow is not connected, every sync flag is off), and equally **do not say the whole system is unbuilt** (the connection foundation, outbound queue, event-operation layer, worker route and cron registration are all built, deployed and were exercised once on 2026-07-18). Only **inbound-busy import and true two-way sync** are unbuilt. A read-only ICS feed also exists.
- **[SUPERSEDED 2026-07-27]** ~~"Comprehensive automated coverage exists."~~ — coverage is now substantial and this ban no longer applies as written: the Vitest unit suite, the Supabase-local DB/RLS integration lane, the generated-types drift check, and **browser E2E** (`playwright.config.ts` plus 52 specs under `e2e/`, run as the `browser-e2e` CI job alongside `payment-browser-e2e`, `mobile-completion-e2e` and `google-browser-e2e`) all exist. **What is still true and worth saying:** coverage is not exhaustive — the E2E lanes are Chromium-only and deliberately exclude real provider sends (Resend/Twilio), real Stripe Elements and real webhook delivery, so manual smoke (docs/12) remains complementary. Say that, rather than claiming either "comprehensive" or "minimal".
