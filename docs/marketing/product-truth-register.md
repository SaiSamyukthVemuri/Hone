...
| Client tolerance + skin/reaction per settings block (with numbing record) | LIVE_FOR_ALL_ONBOARDED | MARKET | Capture how a treatment was tolerated and any reaction, plus whether numbing was used — recorded on the settings block covering the areas treated, as factual records. Chart areas in separate blocks when each needs its own response. |
