...
| V12 | Photos stored privately, camera metadata stripped, short-lived links | home 7, charting | `lib/images/treatment-image-sanitize.ts` re-encodes without metadata so EXIF/GPS/XMP/ICC are stripped before storage; signed-link TTL is **60 s** (`TREATMENT_IMAGE_SIGNED_URL_TTL_SECONDS`, defined in `lib/images/treatment-images.ts`), capped at 300 s by `tests/lib/images/treatment-images.test.ts`. Both are cited as paths because an identifier is not evidence a staleness comparison can watch: production could raise the TTL past the claimed bound with this row still reading as current |
