...
| **Production migration max** | **0160** — 159 migrations applied, each exactly once, no duplicate, repaired or reverted entry. **`0159` (signed-clinical-record retirement) and `0160` (immutable clinical lineage) were both applied and independently verified on 2026-07-30**, with `0159` immediately preceding `0160`. **Hosted max is `0160`; repository max is `0161`.** `0158` is deliberately skipped and will never be applied — DRAFT PR #481 holds a different, superseded `0158` on a retained branch. `0161` (service order RPC + widened calendar-colour CHECK) exists in the REPOSITORY on DRAFT PR #487 but is **NOT APPLIED**: hosted max stays `0160`, repo max is `0161`, and applying it needs separate migration authorization. See §3, [migration-ledger.md](./migration-ledger.md) and the decision record. |
