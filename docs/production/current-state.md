...
## Google Calendar — sync status (as of 2026-07-12)
...
**Google Calendar Phase B1 is deployed as dormant infrastructure.** Migration 0124 added the event-link and durable outbox foundation, but **no worker, enqueue path, trigger, Google event write, inbound busy import, or availability change exists yet.** Sam's controlled Phase A connection remains present; **all synchronization flags remain OFF**; **Willow is not connected.**

| Capability | Designed | Deployed | Enabled (any studio) | Production-exercised | Approved for Willow |
|---|---|---|---|---|---|
| **Phase A — connection & OAuth foundation** (migrations 0121/0122, PR #404) | yes | yes | **connection flag ON for Sam's controlled studio only** (all sync flags OFF) | **yes** — OAuth connection exercised once on Sam's controlled account (1 connection; least-privilege scopes; no event scope) | no |
| **Phase B1 — dormant outbound-sync schema & queue** (migration 0124, PR #407) | yes | yes | no — all sync flags OFF for every studio | **no** — `calendar_event_links` + `calendar_sync_outbox` are empty (0 rows) | no |
| Outbound event sync (Hone → Google worker/enqueue) | yes (design) | **no — not built** | no | no | no |
| Inbound busy / two-way edits | yes (design) | **no — not built** | no | no | no |

**Deployed (Phase B1, migration 0124, PR #407, dormant):** `calendar_event_links` (Hone-entity → Google-event mapping) + `calendar_sync_outbox` (durable at-least-once queue) + the service-role-only `claim_calendar_sync_op` / `record_calendar_sync_result` RPCs. **No behavior ships:** no worker, no enqueue path, no appointment/blocked-time trigger writes to the outbox, no outbound-sync UI, no Google event created or modified, no Google API call, no event scope requested.
...
- **No runtime route or worker references the outbox; no trigger enqueues jobs; no event-sync behavior is enabled.**

**Existing production data is unchanged by 0124:** appointments **75**, timed blocks **9**, calendar reservations **67**; no appointment/booking behavior changed; **no Google event data exists**; no sync job was created; no availability logic changed. Full architecture: [../integrations/google-calendar-sync.md](../integrations/google-calendar-sync.md).
