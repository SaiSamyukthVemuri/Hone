...
- which of those merges are runtime-bearing, by **A3's composed deployed predicate** rather than
  ...
  *not in a non-shipping root*. The classifier answers "is this documentation", which is a
  different question from "does this ship": it returns `docs_only=false` for `.github/workflows`,
  `scripts/` and `e2e/` alike, none of which Vercel serves;
