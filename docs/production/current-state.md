...
- **Last runtime-bearing production HEAD:** `89e77f0063f97083104ce30afed75c951eb79481` — PR #434 merge (**Move appointment owner custom-time override** — a new live capability). Runtime chain since the responsive Move (PR #431, merge `ca7a708`): **PR #433** (cancellation-email actor naming — live) → **PR #434** (owner custom-time). The Google Calendar phases #428 (B2.3-c1) / #429 (B2.3-c2) / #430 (B2.3-c3) merged **dormant** (no runtime capability, no activation). Later documentation-only commits may exist **above** this SHA **without changing application runtime behaviour**.
- **Last runtime-bearing Vercel production deployment:** `HiqfCZU2NYSwL3wWw8Nb2hGEKvam` — status **success** (PR #434 Move custom-time). A later documentation-only branch deployment does **not** indicate a new runtime capability or activation.
- **Move appointment (practitioner) — LIVE production capability (PRs #431 + #434).** A practitioner can move a `confirmed`, future appointment to a new time as an **atomic same-record** update via the migration-**0133** RPC `practitioner_move_appointment`. It is **not** a cancel + rebook: the appointment **ID** and its **client, service, practitioner, payment, clinical (session), intake and calendar-event-link relationships are preserved** — only `starts_at`/`ends_at` + the trigger-owned derived scheduling fields change. **One shared responsive workflow** (`MoveAppointmentDialog`) drives **mobile, tablet and desktop** — no separate paths. **Two modes, both through the SAME atomic 0133 RPC:**
  - **Available times** — for every active practitioner authorized to move the appointment. The dialog shows generated slots and the server **revalidates the submitted time against the current, server-recomputed slot list by start instant** (browser state is not proof); a time that is not currently offered is refused before the RPC.
  - **Custom time** — **studio-owner only**, authorized on the **live server-resolved practitioner role** (never a browser-supplied `isOwner`/role). The owner enters a studio-local time that **may be outside regular operating hours**, and must give an explicit **availability-override acknowledgement** (*"I understand this time overrides regular availability."*). Custom time overrides **only** published availability — it does **not** bypass **confirmed-appointment, appointment-buffer, timed-block, recurring-break or full-day-blockout** conflicts (the GiST `23P01` still rolls a conflicting move back, leaving the appointment unchanged), and it never changes client / service / practitioner / status / duration.

  The custom-time follow-up (**PR #434**, merge `89e77f0063f97083104ce30afed75c951eb79481`, Vercel prod `HiqfCZU2NYSwL3wWw8Nb2hGEKvam`, merge-CI `29602010628` all five lanes green) is **code-only: NO migration was added or applied** — production migration max **remains 0133** (hosted == repo; **no `0134`**). Read-only post-deploy verify PASSED: **no production appointment was moved during deployment verification** (`appointment_audit action='moved'` count unchanged; appointment count unchanged; **no duplicate appointment**), **Google Calendar `worker_enabled=false` and all outbound/inbound/two-way sync flags remained OFF**, **Willow remained unconnected**, **no real Google appointment-event operation occurred**, and **no Stripe or payment state changed**. The **PR #433 cancellation-email actor naming remains deployed**. **Current product state: deployed, awaiting Chloe's controlled custom-time human test.**
