...
- **Production migration max:** **0117** (`0117_session_audit_cross_tenant_insert_hardening.sql`) — local repo max == remote (all applied). 0117 tightened the `session_audit` INSERT policy to bind a new audit row's session to the caller's studio, closing a confirmed cross-tenant integrity-write (Studio A → Studio B session audit). Policy-only; 0 historical mismatches; no code change. 0116 dropped the raw `practitioners.calendar_feed_token` column (calendar-feed credential is hash-only at rest; `calendar_feed_token_hash` retained); existing subscriptions preserved, no forced reconnect.
