...
| **B2.3-c2 — authenticated worker-drain route** (open PR, no migration) | yes | **authored, unmerged — NOT deployed, NOT scheduled** | no — worker OFF, unscheduled | no — dormant, no Google call | no |
| **B2.3-c3/c4/c5 — scheduling / Sam-only activation / soak** | yes (design) | **no — not built** | no | no | no |
