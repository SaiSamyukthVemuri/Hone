...
- **Last runtime-bearing production HEAD:** `ca7a708d3cd5890aa989b78129dcc8471108bafc` — PR #431 merge (**practitioner Move appointment** — a new live capability). The intervening Google Calendar phases #428 (B2.3-c1) / #429 (B2.3-c2) / #430 (B2.3-c3) merged **dormant** (no runtime capability, no activation). Later documentation-only commits may exist **above** this SHA **without changing application runtime behaviour**.
- **Last runtime-bearing Vercel production deployment:** `DwcvmJVXyfLc2JGypU7RuvegzgVp` — status **success** (PR #431 Move appointment). A later documentation-only branch deployment does **not** indicate a new runtime capability or activation.
