...
## Current state (verified 2026-09-06, post-0191 apply; `0192`, `0193` and `0194` authored and PENDING on this branch)
...
| **Repo migration max** | **0194** — this branch carries `0192_waitlist_recipient_proof_authority.sql` (WAIT-03B), `0193_waitlist_admission_authority.sql` (WAIT-ADMIT-01) and `0194_studio_sms_sender_outbound_lookup.sql`, so the repository sits **three migrations above hosted** here. **`0192` is PENDING**, **`0193` is PENDING** and **`0194` is PENDING**: none is applied to production, and none is authorized for apply — that is a separate gate. Hosted remains **0191** and the apply record above is unchanged; this row states the BRANCH-derived position, not a new production claim. **`0194` IS EXTRACTED FROM #674, NOT AUTHORED HERE.** Its SQL is byte-identical to #674's (`sha256 b4a40929acf4e7d291b1752ea1615aa63f1ee19120a33906b3bfa67ab6ce725b`), taken so the DATABASE CAPABILITY can land independently of SMS runtime activation. **#674 is not merged and is not obsolete** — it remains the owner of sender-routing activation, `sendSmsSafely`, the Twilio path and the cron routing. Nothing here enables any of them. An earlier revision recorded `0192` as spoken for by that migration on #674; that was resolved by renumbering it to `0194` on #674's own branch, which is the number carried here. Next free number is **0195**, derived from **this tree's** `supabase/migrations/*.sql` by `npm run migration:state` and **not claimed** by this lane. The derivation sees only this working tree and is **not** a statement about any other branch. |
| **Remote-only migrations** | **none** — no migration exists on production that the repository lacks |
| **Total migrations in repo** | **193** (`0001` … `0157`, `0159` … `0194` — **no `0158`**) — `0001`–`0157` is 157 and `0159`–`0194` is 36. Derived by `npm run migration:state` on this branch, which reports **193** with `0192`, `0193` and `0194` all pending. This row moves with the repo max above it: leaving it behind made the ledger contradict the very command it cites, and neither `verify:prepush` nor the docs guard compares the total against the derived value. |
