...
| **Application merge** | **At the time of this apply:** #593 was still open and application production was `a1047c7a5fdcad58bf033b6ae1cdcc8f7f1b5875`, so production briefly held the budget schema with no UI writing to it — the intended migration-first ordering, not a defect. **Subsequently merged** as `266b6092f22ffd6d656a967be527abcf9437a95f` (`2026-08-17T12:40:47Z`; normal two-parent merge, parent 1 `a1047c7a` = previous production, parent 2 `85b7922d` = the authorized head), and the Vercel production deployment for that SHA succeeded. *Current* application-deployment status is deliberately not tracked here — **read it from the live systems** (the production branch head on GitHub, and the Vercel production deployment). [current-state.md](./current-state.md) and [release-changelog.md](./release-changelog.md) are the intended long-form homes, but **as of this record both are STALE**: each was last reconciled **2026-07-27** and still names `96b28d6` (PR #478) as the runtime-bearing head, and **neither mentions #593 or the `266b6092` merge**. Their stated production migration maxima **differ from each other and both are stale**: `current-state.md` says **0165**, `release-changelog.md` says **0157**, and production is actually **0184**. Bringing them current is an **open follow-up**, not done here. |
| **Where the apply ran** | **`hone-dev-01`, the Hone repository host — ESTABLISHED for 0184.** An earlier capability recon of this host concluded **both** applies ran on a separate operator machine. That conclusion is **WITHDRAWN** — it is replaced below by what the artifacts actually support, and no further. `supabase/.temp/pgdelta/` was modified `2026-08-17T12:03:00Z`, **inside** this apply window (`12:02:40Z`–`12:03:01Z`), and `pgdelta` is written by `supabase db push` — a direct artifact of the push itself, on this host. **Credentials were TEMPORARY and environment-scoped**, supplied to the release shell only: after the release `SUPABASE_ACCESS_TOKEN` and `SUPABASE_DB_PASSWORD` were both unset, no token exists on disk, and nothing was committed — `supabase/.temp/` is gitignored (`.gitignore:45`) and untracked. The linked `project-ref` that remains grants **no** database authority without a valid token or password. |
...
| **Applied from** | the authorized #593 reviewed head `e43330a0c2cf8b84dac4f8538611febce4f013f1` — **DATABASE FIRST, before any application merge.** #593 was still open at this point; it merged later as `266b6092f22ffd6d656a967be527abcf9437a95f`. |
| **Application merge** | **None at the time of this apply.** The feature's application code shipped later, with #593's merge `266b6092f22ffd6d656a967be527abcf9437a95f`. |
| **Where the apply ran** | ⚠️ **INFERENCE, NOT ESTABLISHED.** An earlier capability recon of this host concluded **both** applies ran on a separate operator machine. That conclusion is **WITHDRAWN** — it is replaced below by what the artifacts actually support, and no further. `supabase/.temp/project-ref` and `linked-project.json` were created `2026-08-17T01:00:02Z`, ~3 min before this window opened at `01:03:11Z`. That proves this host was made **capable** of applying, and makes it the likely origin given 0184 demonstrably ran here — but it does **not** prove the 0183 `db push` itself ran here: the operator could have linked here and pushed from elsewhere. **No command, process or per-apply artifact ties this window to this host, and none is invented.** **Credentials were TEMPORARY and environment-scoped**, supplied to the release shell only: after the release `SUPABASE_ACCESS_TOKEN` and `SUPABASE_DB_PASSWORD` were both unset, no token exists on disk, and nothing was committed — `supabase/.temp/` is gitignored (`.gitignore:45`) and untracked. The linked `project-ref` that remains grants **no** database authority without a valid token or password. |
