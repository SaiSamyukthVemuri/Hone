...
| **Repo migration max** | **0192** — **this branch carries `0192_studio_sms_sender_outbound_lookup.sql` (COMMS-01B2)**, so the repository sits **one migration above hosted** here. **`0192` is PENDING**: not applied to production, and not authorized for apply — that is a separate gate. Hosted remains **0191** and the apply record above is unchanged; this row states the BRANCH-derived position, not a new production claim. Next free number is **0193**, derived from **this tree's** `supabase/migrations/*.sql` by `npm run migration:state` and **not claimed** by this lane. The derivation sees only this working tree, so it records availability *here* and is **not** a statement that `0193` is free on any other branch or worktree — another lane may already have authored it, and this one has not looked. |
| **Remote-only migrations** | **none** — no migration exists on production that the repository lacks |
| **Total migrations in repo** | **191** (`0001` … `0157`, `0159` … `0192` — **no `0158`**) — derived by `npm run migration:state` on this branch, not incremented by hand. It is **one above** the **190** hosted history rows precisely because `0192` is authored here and unapplied; the apply record above remains the authority for what production holds. |
