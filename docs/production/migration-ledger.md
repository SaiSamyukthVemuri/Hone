...
| **Repo migration max** | **0199** — this branch authors `0199_reminder_sms_candidate_selection.sql` (WAIT S3, bounded server-side reminder candidate selection): TWO read-only `SECURITY DEFINER` functions, service_role-only, no table, column, index, backfill or data write, and **no widening of `studio_sms_senders`**, which stays closed to every role exactly as `0191` left it. **`0199` is PENDING**: authored here, **not applied**, and not authorized for apply — that is a separate gate. Hosted remains **0198** and the apply record above is unchanged; this row states the BRANCH-derived position, not a production claim. The number was derived by `npm run migration:state` from this tree, and when enumerated on 2026-09-17 **no pushed branch (0 of 710 refs), open PR, local worktree or stash held an `0199`** — a branch pushed after that reading is not covered by it. |
