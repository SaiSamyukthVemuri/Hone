...
| Charting | Legacy `reaction_type` folded into the unified representation | Merged | — | Deployed | Enabled | ✅ historical rows read through the fold on every render | ✅ | `lib/observation-chips.ts:221-225` | — |
| Charting | Reaction-driven analytics consume the unified representation | Merged | — | Deployed | Enabled | ✅ dashboard surfaces render from it | ✅ | `lib/dashboard/clients-needing-attention.ts:126` | — |
| Charting | **Galvanic intensity retired** from current writes and ordinary display | Merged | no column drop — data preserved | Deployed | n/a (retired) | ✅ new rows write NULL; forged specs ignored | **Pending** | `block-actions.ts:718,816` ("a RETIRED reading… intentionally NOT emitted") | Historical values are preserved, not deleted. `galvanic_ma` and `galvanic_duration_seconds` remain **active** readings |
...
| Whole-session copy | Zero writes before the explicit commit | Merged | — | Deployed | Enabled | ✅ implied by the 0-row ledger | **Pending** | 0 ledger rows despite the feature being live | Preview is component memory only |
...
| Payments | Card add/replace studio notification (0154) | Merged | ✅ applied | Deployed | Enabled | ✅ 30 `practitioner_notifications` present | ✅ | migration 0154; dedupe on the mode-scoped SetupIntent | — |
...
| Google Calendar | B2.3-c1 — event-operation layer + transition RPC (0132, PR #428) | Merged | ✅ 0132 applied | Deployed | worker off | see c2/c3 | — | migration 0132 | PR #428 metadata should be re-checked before relying on its merge state |
...
| Google Calendar | B2.3-c3 — cron schedule registration (PR #430) | Merged | no migration | Deployed | **3 daily crons registered in `vercel.json`** (`materialize-recurring-breaks` 08:00, `calendar-reconcile` 09:00, `calendar-sync` 09:30) | ⚠️ they run and find no work | — | `vercel.json` | Registration did **not** activate sync — the flags do that |
...
| Multi-practitioner | Per-practitioner availability + scoped blocks/breaks (0135, 0137–0139) | Merged | ✅ applied | Deployed | follows the capacity flag | ⚠️ test studio only | — | migrations 0135–0139 | — |
| Multi-practitioner | Internal booking + move/reassign commands (0142–0150) | Merged | ✅ applied | Deployed | follows the capacity flag | ⚠️ test studio only | — | migrations 0142–0150 | Authoritative in-DB duration; owner-only length override |
...
| Ops | Cron routes with `CRON_SECRET` | Merged | — | Deployed | 3 daily calendar crons registered, worker off | ⚠️ | — | `vercel.json` | — |
...
|---|---|---|
| Deployed + enabled + production-exercised + human-accepted | ~20 | booking, charting core, portal, intake, consent, photos, live session payments, record keeping |
