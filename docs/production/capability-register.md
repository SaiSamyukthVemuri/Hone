...
commit point is an email. WAIT-02B Stage A is deployed, its **public** commit point is enabled for no studio, and — ⚠️
**corrected 2026-09-17** — its **owner** surface IS reachable: `/settings/waitlist` writes
durable rows through `0193`'s owner commands without consulting the durable allowlist. This
line read *"deployed and reachable by nobody"*, which stopped being true when that surface
shipped. See [current-state.md](./current-state.md) §5b.
