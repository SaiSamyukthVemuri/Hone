...
| **Owner** | Sam |
| **Status** | 🔴 **OPEN IN PRODUCTION.** Every statement above describes **live production behaviour today.** `0172` is now merged and applied (2026-08-09, hosted max `0172`), and it closed **direct** appointment DML — but it deliberately did not touch this edge, because a foreign-key referential action runs as the *constraint's* owner and consults neither the table ACL nor RLS. A member can still delete a `services` row and null `appointments.service_id`; an owner can still delete a `practitioners` row and null `appointments.practitioner_id`; both writes remain silent. **The closure is implemented and locally/CI-proven in B4 / migration `0173` GROUP 5, but remains UNAPPLIED until the separate `0173` production ceremony** — PR #534 is open, draft and unmerged, and hosted migration max is `0172`. `0173`'s GROUP 5 was briefly drafted as a companion migration `0174` and withdrawn: the canonical appointment-DML program reserves `0174` for B5 (attribution + audit integrity), `0175` for B6, `0176` for B7 and `0177` for B8, so consuming `0174` would have shifted every later boundary migration by one. B4 therefore ships exactly **one** migration. Do not read this row as "fixed" until `docs/production/migration-state.json` records `0173` as applied. |
...
| **Blocks** | **Was not a blocker for `0172`**, which shipped on 2026-08-09: it strictly reduced the attack surface and introduced none of this — the paths predate it and are unchanged by it. It is recorded here because `0172` is what made the remaining edge worth naming. It is now the **principal residual risk on the appointment boundary in production**, and it is what the `0173` apply ceremony closes. |
