...
|---|---|
| **Recorded** | 2026-07-29 |
| **Status (2026-08-03)** | **ALL RUNTIME WRITERS ARE ZERO. The revocation is prepared but NOT APPLIED — L18 REMAINS OPEN.** |
| **Impact** | A studio member's browser JWT can still `INSERT`/`UPDATE`/`DELETE` `public.sessions`, `public.session_blocks`, `public.electrolysis_entries`, `public.laser_entries` and `public.treatment_images` **directly through PostgREST**, bypassing every application command. RLS restricts it to the member's own studio and 0160 pins the lineage columns, so this is not a cross-tenant or cross-client hole. What it allows is a member editing clinical rows outside the reviewed commands, with none of their validation, defaulting or audit behaviour. **No application code does this any more — but the capability is still granted.** |
| **Evidence** | `has_table_privilege('authenticated', …)` verified against production 2026-08-03: `sessions` and `session_blocks` = `SELECT/INSERT/UPDATE/DELETE`; `electrolysis_entries`, `laser_entries`, `treatment_images` = `SELECT/INSERT/UPDATE`; `session_block_areas` = `SELECT` only. Root cause: Supabase's `ALTER DEFAULT PRIVILEGES` granted these at table creation and no migration in 0001–0157 ever named `sessions` or `session_blocks` in a grant or revoke at all. |
| **Writer census** | **25 → 0.** `sessions` 10→0, `session_blocks` 7→0, `electrolysis_entries` 4→0, `treatment_images` 3→0, `laser_entries` 1→0, `session_block_areas` 0→0. Measured by statement-chain walk (reads excluded); pinned by `tests/security/entry-direct-dml-guard.test.ts`, which carries **no exception list**. |
| **Command boundary (APPLIED)** | `0164`+`0165` (laser), `0166` (blocks + electrolysis entries), `0167` (sessions), `0168` (treatment images) — **16 public commands**, all `SECURITY DEFINER` with `search_path=""`, `authenticated`-EXECUTE only, actor and studio derived from `auth.uid()`. Every one of those migrations was **additive** and revoked nothing. Full record: [l18-command-inventory.md](l18-command-inventory.md). |
| **Revocation (`0169`, PREPARED — NOT APPLIED)** | Revokes `INSERT`/`UPDATE`/`DELETE` from `authenticated` on the six tables. **`authenticated` SELECT is RETAINED**; `service_role`, `anon` and PUBLIC are unchanged; no `REVOKE ALL`; no function EXECUTE, policy, trigger, schema or data change. `session_block_areas` is included as a deliberate no-op so the posture is explicit for all six. All 16 command suites (126 behavioural cases) pass against a chain that includes `0169`. |
| **Verification honesty** | **Production behavioural write-probing was never performed** — the `db query` classifier blocks INSERT/UPDATE-bearing SQL, so the commands are source- and privilege-verified in production and never behaviourally exercised there. Behaviour is proven on a fresh local stack and in the browser lane only. |
| **Owner** | Sam |
| **Next gate** | Apply `0169` to production, then merge its PR and confirm the deployment. Nothing else remains. |
