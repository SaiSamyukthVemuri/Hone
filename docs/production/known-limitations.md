...
| **Not the whole picture for new-client intake** | A **new-client waitlist** exists and **was live at Willow at every measured instant**: new-client booking was refused and routed to a waitlist. ⚠️ **CORRECTED 2026-09-19.** This entry recorded the commit point as the studio notification email, and described the **durable** path as deployed but inert, switched on for no studio. Both were true at WAIT-01 activation (PR #601, 2026-08-19) and stopped being true on or before **2026-08-25**, when the durable commit point was activated for that studio. Willow's waitlist joins committed to `new_client_waitlist_entries` at every measured instant (WAIT-02B Stage A, migration 0185) — **31 rows measured 2026-09-19**, 28 of them Willow's. At those instants the email was a notification and the row the record; **which path carries a join after 2026-09-15 depends on allowlist membership, re-read per request** — see **L25**. Read this limitation as "no direct route", **not** as "nothing exists for new clients". |
| **Mitigation, as measured** | Existing public booking, the **durable** new-client waitlist at Willow *(corrected 2026-09-19; this said WAIT-01)* — **which carried Willow's new-client intake at every measured instant, 2026-08-25 through 2026-09-15** — and operator-side booking cover pilot volume. ⚠️ Which of the two waitlist paths carries that intake **now** depends on allowlist membership, which is re-read per request and is not derivable from persisted rows. |
| **Owner** | Product (Sam) |
...
| **Owner** | Sam |
| **Next gate** | **Level 3 sequence:** WAIT-04B prospect mobile verification + phone-wide STOP/suppression authority → product-operable studio sender provisioning/adoption/test → verified Willow studio sender → fixed/no-choice 48h policy → email + eligible SMS WAIT delivery → WAIT 24h reminder → only then consider the #716-style no-fallback appointment-SMS routing cutover. A2P/10DLC and scale controls remain required before broader multi-studio SMS rollout. |
...
| **Evidence (historical, 2026-08-23)** | `NEW_CLIENT_WAITLIST_DURABLE_STUDIO_SLUGS` was **absent from the Vercel Production environment** *(verified 2026-08-23, variable **names** only; no value read)*, and `new_client_waitlist_entries` held **no rows** at apply verification and when last measured that day. **That observation is preserved as dated evidence and is NOT the current state.** |
...
| **What is NOT mitigated** | ⚠️ **CORRECTED 2026-09-19.** This row previously recorded Stage B2 activation as withheld, with none enabled and no activation smoke run. **The first two of those are contradicted by the measured evidence** — activation was taken on or before 2026-08-25 and one studio is enabled, as the rows above record. What is NOT mitigated is the **missing governance record**: no release record, ledger entry or activation-smoke result exists in this repository, and whether a smoke was run off-repository is **not established and is not asserted either way**. |
...
- ⚠️ **INVERTED 2026-09-19.** This bullet used to forbid claiming the durable new-client
  ...
  **Production disproved that on 2026-08-25.** The bullet now forbids the opposite error: never
  ...
  the durable join path was live and taking real prospects at every measured instant, while every later stage is untouched —
