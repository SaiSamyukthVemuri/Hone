...
| **What is NOT mitigated** | ⚠️ **CORRECTED 2026-09-19.** This row previously recorded Stage B2 activation as withheld, with none enabled and no activation smoke run. **The first two of those are false** — activation was taken on or before 2026-08-25 and one studio is enabled, as the rows above record. What is NOT mitigated is the **missing governance record**: no release record, ledger entry or activation-smoke result exists in this repository, and whether a smoke was run off-repository is **not established and is not asserted either way**. |
