...
| **Phase 1A (`0164` + `0165` APPLIED 2026-08-02)** | PARTIAL — the clean laser-entry creation path uses a narrow command. Electrolysis entry writers remain direct because each relevant user workflow can depend on `session_blocks` and must move atomically in the combined phase. Direct table grants remain in place. `addLaserEntryAction` now calls `create_laser_entry` — SECURITY DEFINER, `search_path=''`, non-null `auth.uid()` required, studio and client derived from the trusted `sessions` row. **All three electrolysis writers remain direct and are deferred**, including `addElectrolysisEntryAction`: it can create a default `session_blocks` row through `ensureBlockForSession` before creating the electrolysis entry; the two writes are not atomic today and must move together. ⚠️ **`0164` shipped with an unintended `service_role` EXECUTE grant** (Supabase default privileges; `0164` revoked only `public`+`anon`) — discovered after the apply, **no exposure found** because the command requires a non-null `auth.uid()`. **Repaired by `0165`, APPLIED 2026-08-02T20:20Z** — the `create_laser_entry` ACL is now exactly `{postgres, authenticated}`. Verified writer census: 25. |
| **Phase 2 (migration `0166`, WRITTEN — NOT APPLIED, application NOT migrated)** | `0166` adds four narrow SECURITY DEFINER commands for the coupled block/entry workflows — `create_block_with_entry`, `update_block_with_entry`, `add_electrolysis_pass`, `soft_delete_session_block` — each performing its whole workflow in ONE transaction, so the block mutation, area linkage and entry mutation roll back together. The block+areas half delegates to the existing 0128/0129 commands rather than opening a competing area-write path. Actor is derived from `auth.uid()` (including `deleted_by`); no studio or actor parameter exists. EXECUTE: `authenticated` only for the four commands; the three internal helpers are revoked from `authenticated` too. **⚠️ The application writers are NOT yet migrated** — the verified census is unchanged at `session_blocks` **7** and `electrolysis_entries` **4** direct runtime writers. The commands exist and are proven by 26 DB tests but nothing calls them yet, so Phase 2's acceptance target of zero direct writers is **NOT met**. `0166` is additive and revokes nothing, so it is safe to apply ahead of the application change. **L18 remains OPEN.** |
