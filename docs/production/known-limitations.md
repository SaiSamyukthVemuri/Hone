...
| **Owner** | Sam |
| **Status** | ⚠️ **CLOSED IN CODE — NOT YET APPLIED TO PRODUCTION.** **GROUP 5 of `0173_appointment_repair_commands.sql`** (appointment boundary B4, draft PR #534 stacked on B3) takes the first-preference option below. It was briefly drafted as a companion migration `0174` and withdrawn: the canonical appointment-DML program reserves `0174` for B5 (attribution + audit integrity), `0175` for B6, `0176` for B7 and `0177` for B8, so consuming `0174` here would have shifted every later boundary migration by one. B4 therefore ships exactly **one** migration. It is built and green on a fresh local `0001 → 0173` chain; **neither `0172` nor `0173` has been pushed to production**, so in production L23 remains fully OPEN and every statement above still describes live behaviour. Do not read this row as "fixed" until the migration state records `0173` as applied. |
