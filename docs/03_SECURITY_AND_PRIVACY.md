...
| `/login` (practitioner) | Yes | **Invite-only during the pilot (PR #189).** The magic-link request runs through a server action that sets `shouldCreateUser` true ONLY when a pending invitation exists for the email; uninvited unknown emails get the same generic "sent" response and no auth user or studio is created. Known residual: the Google OAuth button cannot pass `shouldCreateUser` and still creates an auth user (and, via the `handle_new_user()` no-invite fallback, a fresh studio) on first OAuth login; closing that requires disabling the provider or a Supabase auth hook. |
