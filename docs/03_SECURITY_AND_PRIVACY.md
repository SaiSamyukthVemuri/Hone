...
| New-client waitlist prospect data | **Partially closed by WAIT-02B Stage B1. PRODUCTION STILL ENABLES ZERO STUDIOS.** Migration 0185 creates `new_client_waitlist_entries`, a studio-scoped personal-data class (name, email, optional phone for people who are **not** clients and who did not have their details entered by a practitioner). Stage A recorded three open items; Stage B1 closes the first and leaves the other two open, deliberately and without glossing them. **(1) CLOSED — the public privacy notice now covers it.** `app/privacy/page.tsx` names **Prospective clients** as a covered category in §2, describes what the waitlist form collects in §3 (“From prospective clients directly”), states the purpose in §4, states that the request goes to that one studio and no other in §6, gives an account-free removal/access route in §8, and describes waitlist retention truthfully in §9. **§6 distinguishes the two commit points rather than claiming one for everybody**: a studio whose waitlist is kept with us gets a stored entry that no email failure can retract, and a studio still on the WAIT-01 notification path gets no record on Hone's side at all — for which the policy says so, says the email *is* the request, and says a submission that cannot be sent is reported as failed rather than as joined. An earlier draft asserted durable storage for every request, which was false for every studio in the shipping configuration, since the allowlist names none. The public waitlist form carries the short version next to its submit control, with a link to the policy. Both are pinned by `tests/app/privacy/waitlist-prospect-disclosure.test.ts`. **(2) STILL OPEN — not in the `/settings/data` studio export.** The export page says “supported studio records” rather than claiming completeness, so nothing in the product asserts otherwise, but a studio asked for “everything you hold about me” would not produce these rows. **(3) STILL OPEN — no timed purge.** Removal in the product is a terminal `removed` transition that retains the row. The policy now SAYS SO rather than implying a retention period Hone does not operate; it does not invent one. Closing (2) and (3) belongs with the export/offboarding slice (PRIV-01). **Activation remains an explicit operator step**, per studio, via `NEW_CLIENT_WAITLIST_DURABLE_STUDIO_SLUGS`, against a deploy-time shape check that refuses a list which cannot name a real studio. It has not been taken. |
