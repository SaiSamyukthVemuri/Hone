...
| New-client waitlist export / offboarding | **Open, and deliberately not claimed.** Migration 0185 creates `new_client_waitlist_entries`, a NEW studio-scoped personal-data class (name, email, optional phone for people who are **not** clients). It is **not** included in the `/settings/data` studio export, and no retention or purge policy covers it yet. The export page already says "supported studio records" rather than claiming completeness, so nothing in the product asserts otherwise — but a studio asking for "everything you hold about me" would not get these rows today. Integrating it belongs with the export/offboarding slice (PRIV-01), which is also where a physical purge would live; removal in the product is a terminal `removed` transition that retains the row. |
