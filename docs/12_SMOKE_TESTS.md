...
## Quick Import smoke (PR #257, superseded in part by IMPORT-01 2026-08-12)

**Current truth, in one line: an ordinary studio owner cannot run an import. Hone runs it for them.** The route, the pipeline and the operator flow all still exist; what changed is who may execute them. The three parts below are the whole contract — part 1 is what a customer sees, part 2 is what a Hone operator does, part 3 is what is still broken. Nothing in part 2 applies to an ordinary owner.

### 1. Ordinary studio owner — the current production contract

`/settings/import` is an **informational surface only**. Signed in as a studio owner who is *not* on the `ADMIN_EMAILS` allowlist, confirm:

- The page heading is **"Import clients and history"** and it shows the banner **"Import is currently operator-assisted"**, an explanation, the column template to prepare, and a **Contact support** link to `mailto:support@hone.care`.
- There is **no executable control anywhere on it** — no paste box (`#import-text`), no Source picker, no Copy template, no Preview import, no Confirm import. Not a greyed-out decoy either: the controls are **absent**, not disabled.
- **The page is not the control.** The server is. Both actions (`previewImportAction`, `confirmImportAction`) open with `ownerContext()`, which requires an active studio owner **and** platform-operator standing (`isImportOperator()` → `supabase.auth.getUser()` checked against `isAdmin`/`ADMIN_EMAILS`, fail-closed in production) **before the first statement of either**. A direct POST to either action, bypassing the page entirely, is refused on identical terms and **writes nothing**.
- **Global Search** for `import` / `csv` / `quick import` / `migrate` returns **"Import clients and history — Operator-assisted: Hone brings your existing records over for you"**. The old "Quick import" title is gone. The entry stays owner-only; a non-owner practitioner gets nothing for any of those queries.
- `/settings/data` → the **"Import clients from CSV"** card says operator-assisted and links to `/settings/import`. It no longer says "Coming this week".
- Unchanged access gates: anonymous `curl -sI https://hone.care/settings/import` → `307 -> /login`; a no-studio user → `/no-access`; a non-owner practitioner sees "Only studio owners can import clients and treatment history."

Pinned by `tests/app/settings/import/operator-assisted-gate.test.ts` (real actions, recording stub, **zero writes** on denial, allowlist near-misses, production fail-closed), `tests/app/settings/import/quick-import-action.test.ts` (per-function gate-before-write pins), `tests/lib/search/import-search-truthfulness.test.ts`, and `e2e/quick-import.spec.ts`.

### 2. Platform operator + active studio owner — the assisted migration flow

Visible **only** to a caller who is both on `ADMIN_EMAILS` and an active owner of the studio being written to. The operator writes through the same RLS-backed authenticated client as before — **no service role** — so they can only import into a studio they actually own.

> ⚠️ **This flow is still NOT ATOMIC.** It is three independent statements with no transaction and no RPC: `import_batches` → `clients` → `imported_treatment_memories`. If the third fails, the second has already committed; migration 0087 forbids hard-deleting a client, so the batch is soft-voided and **the created client rows stay**. Re-running the identical file matches those rows as confident duplicates and **skips** them, so the same file cannot repair the history it failed to write. Use it for a **deliberate, supervised, real migration only**, and reconcile the summary counts (clients created / memories created / duplicates skipped) with the source file before telling the studio it is done. Never use it for a throwaway test.

Flow: Copy template → paste rows (Google Sheets/Excel TSV or CSV; include the header) → pick a Source → **Preview import** (no writes: source rows, grouped clients, ready/review/duplicate counts, treatment areas detected, ignored columns, per-group status chips) → **Confirm import** → summary with links to the created clients.

Safety invariants, all unchanged from PR #257: two rows for the same client (e.g. Upper lip + Chin, same email) group into **one** client with two memory rows; a row matching an existing client by email/phone/name+DOB is **skipped**, never overwritten or merged; a same-name-only match is created with a "Review" warning, never merged; imported history is labelled "Imported history, not charted live in Hone." and never writes `sessions`/`session_blocks`/`appointments`; the raw pasted text is never stored or logged; 2000-row cap; no OCR, AI, API sync, upload, external fetch, email, SMS or payment.

### 3. Root fix still pending

Self-service import stays disabled until a **staged / transactional / resumable** import exists — stage the rows, commit atomically or resume idempotently, plus an **attach-to-existing-client** repair path so a retry can complete a partial import instead of skipping it. That is a separate, migration-owning project. Until it ships, do **not** describe Quick Import as safe or self-service anywhere. Tracked as **L24** in [docs/production/known-limitations.md](./production/known-limitations.md); the decision is recorded under IMPORT-01 in [docs/13](./13_BACKLOG_AND_DECISIONS.md).
...
## Intake reminders at ~24h / ~2h, composed into the window message (migration 0186)

Intake-form reminders no longer run on their own 7d/3d cadence. They fire at the **~24h and ~2h reminder windows** and are **composed into the appointment reminder** rather than sent as a second message. **Migration 0186** adds exactly one column — `studios.send_intake_reminders boolean not null default true` — with no RPC, index, enum, RLS, CHECK, trigger or destructive change; it creates no function. The 0098 `intake_reminder_7d/3d_*` columns, indexes and `claim_email_send` / `record_email_result` branches are **retained untouched and simply stop being written** (never reinterpreted).

**The invariant:** whatever the pass composes, it claims the **existing** per-window slot — `claim_email_send('reminder_24h'|'reminder_2h')` for email, `claim_sms_send` for SMS — so there is **at most one email and at most one SMS per appointment per window**. Email and SMS are independent channels, so one of each is allowed.

**Six-case email law** per window (R = that window's reminder toggle, I = `send_intake_reminders`): R+I+incomplete → one reminder carrying the intake CTA; R+I+complete → one plain reminder; R with I off → one plain reminder; R off + I + incomplete → one **standalone** intake reminder; R off + I + complete → **nothing, and no claim**; both off → nothing. **SMS has no standalone case:** the window's own `send_24h_sms_reminders` / `send_2h_sms_reminders` toggle gates it first, so enabling intake reminders can never open a new SMS channel, and phone + `sms_consent_at` + no `sms_opted_out_at` are still required.

**"Incomplete" is exactly** the latest **non-deleted** `client_intake_forms` row for `(studio_id, client_id)` with `status = 'in_progress'`. Submitted/reviewed → no CTA; missing row → skipped (the cron never creates one).

**Manual smoke (read-only — do NOT trigger real sends).** For a confirmed appointment ~24h out whose latest intake is `in_progress`, with a client email on file: the window email carries the intake section (heading "Please complete your intake form", CTA "Complete intake form", a working fresh link) **below the appointment details**, and the subject is the unchanged appointment-reminder subject with no intake wording. Flip the intake to `submitted` and the next window sends a plain reminder with **no** CTA and **no** change to `intake_link_send_count` / `intake_link_last_sent_at`. Turn `send_24h_reminders` off with `send_intake_reminders` on and the window sends the standalone intake reminder, subject "Please complete your intake form" (2h: "A quick reminder about your intake form"). Turn `send_intake_reminders` off and the window sends the plain reminder only. With the window's SMS toggle on and a consented phone, the SMS carries one concise intake sentence and the link in the SAME message as the appointment reminder — never a second SMS. **Lock-screen check:** no subject, preheader or SMS body ever states that the form is incomplete, and none carries clinical or health detail.

**Settings:** `send_intake_reminders` appears in the practitioner **Email notifications** list at `/settings/studio#email-notifications`, alongside the confirmation and 24h/2h reminder toggles. Default ON.

**Appointment detail:** the "Email activity" rows read **"24-hour email" / "2-hour email"** (not "reminder"), because those columns are now the per-window email slot and a standalone-intake send would otherwise claim a reminder was sent that was not.

Pinned by `tests/migrations/0186-intake-reminder-24h-2h.test.ts`, `tests/app/api/cron/reminder-window-composition.test.ts`, `tests/db/reminder-window-composition.db.test.ts` (six cases, concurrency, retry and preservation controls against the real migrated database), `tests/lib/email/reminder-intake-cta.test.ts`, `tests/lib/email/intake-reminder-template.test.ts`, `tests/lib/sms/reminder-intake-cta.test.ts`. Out of scope: preference centre, per-client cadence. Live payments unaffected.
