...
## 0. Marketing homepage smoke (PR #242, tightened PR #243)

The public homepage (`/`) is positioned around treatment memory and, after PR #243, follows a tightened YC-style structure of eight sections: hero, calendar-vs-Hone comparison, How Hone works (before/during/after), product proof (Before Today, charting, procedure records, Daily Prep Brief as compact cards), Record Keeping, one merged Agentic support section, privacy/trust, and pricing/CTA. Open `https://hone.care/` on a phone (~390px) and on desktop and confirm: the hero reads "Treatment memory for permanent hair removal studios." within five seconds with the calendar line "Your calendar tells you who is coming. Hone tells you what to remember." and ONE hero visual (not a stack of competing cards); the page does not scroll sideways at either width; the Book walkthrough CTA is visible and reachable (header button on desktop, menu on mobile) and lands on `/demo`; Sign in is reachable and lands on `/login`; the comparison, How Hone works, product proof, Record Keeping (with the local public-health responsibility caveat), the single "Agentic support, but practitioner-controlled." section, privacy/trust, and pricing ($19/month founding pilot) all render; the nav is short (Product, Records, Agentic support, Pricing, Sign in plus the CTA); all product visuals use only anonymized demo data (Maya R., Demo Studio, lot L-204, Sterex), never a real client or studio; and there are no medical, compliance, or AI overclaims (no HIPAA/public-health-certified/medical-grade/guaranteed-compliance, no autonomous clinical decisions claimed as a feature). The Playwright lane (`e2e/marketing-homepage.spec.ts`) covers the overflow + CTA + sign-in checks at phone and desktop widths.
