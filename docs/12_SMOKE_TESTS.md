...
> **IMPORT-01 (2026-08-12) — self-service execution is OFF.** `/settings/import` is now **operator-assisted only**. An ordinary studio owner reaching it sees an informational surface ("Import is currently operator-assisted", the column template to prepare, and a `mailto:support@hone.care` contact) and **no executable control at all** — no paste box, no Source picker, no Preview/Confirm, and no disabled decoy. The boundary is the **server**, not the page: `ownerContext()` in `app/(app)/settings/import/actions.ts` requires active studio ownership **and** platform-operator standing (`isAdmin`/`ADMIN_EMAILS` via `lib/import/operator-assist.ts`, fail-closed in production) before the first statement of **either** action, so a direct POST to `previewImportAction`/`confirmImportAction` is refused on the same terms and writes nothing. **Why:** the confirm path runs three unwrapped statements (batch → clients → memories); a failure at the third leaves the created **clients** behind (0087 forbids hard-deleting them), and re-importing the same file matches those rows as duplicates and **skips** them, so the studio cannot self-repair. The root fix is a staged/transactional/resumable import and is not built yet. **Operator flow is unchanged** — an operator who is also an active owner of the studio sees the original paste → preview → confirm island (behind an "Operator-assisted import" banner) and writes through the same RLS-backed client; no service role was introduced. **Smoke it (non-destructive):** as an ordinary owner, `/settings/import` shows the operator-assisted notice and zero import buttons; Global Search for "import"/"csv"/"quick import" returns **Import clients and history — Operator-assisted — Hone brings your existing records over for you** (the old "Quick import" title is gone); `/settings/data` → the "Import clients from CSV" card says operator-assisted and links to `/settings/import` (it no longer says "Coming this week"). Pinned by `tests/app/settings/import/operator-assisted-gate.test.ts` (real actions, recording stub, **zero writes** on denial, allowlist near-misses, prod fail-closed), `tests/app/settings/import/quick-import-action.test.ts`, `tests/lib/search/import-search-truthfulness.test.ts`, and `e2e/quick-import.spec.ts`. The rest of this section describes the **operator** flow and the unchanged safety invariants.
