...
Cron schedules now live in `vercel.json` (Vercel Cron), not an external scheduler. **Config check (repo):** `vercel.json` `"crons"` schedules `/api/cron/appointment-reminders` at `*/15 * * * *` and `/api/cron/materialize-recurring-breaks` at `0 8 * * *`; `/api/cron/no-show-check` is intentionally absent (disabled stub). **Auth check (prod):** an unauthenticated `curl -sI https://hone.care/api/cron/appointment-reminders` returns `401` (the route requires `Authorization: Bearer $CRON_SECRET`, which Vercel Cron attaches automatically; the env var must be set in the Vercel project). **Reminder coverage:** the 2h reminder window is `now+105m … now+135m` (30 min); at the `*/15` cadence every appointment minute offset is covered and a single skipped fire still leaves a grid point in-window — proven deterministically by `tests/lib/cron/reminder-schedule.test.ts` (and that suite fails for the old hourly + 30-min combination). **Post-deploy (operator):** after deploy, the Vercel dashboard → Cron tab should list both scheduled jobs with recent successful runs; spot-check the appointment-reminders run logs show `{ ok: true, reminder_24h, reminder_2h, ... }`. If a reminder exhausts 3 attempts a `reminder_send_exhausted` ops alert (warning) is recorded with non-sensitive metadata only (no client email/phone/notes/token). Reminders are only sent for appointments still `status='confirmed'` at send time. Live payments remain disabled.
