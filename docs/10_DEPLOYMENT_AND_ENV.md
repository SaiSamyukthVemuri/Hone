...
**Reminder scheduler alerting (PR #283, hardened by PR OPS-01).** The PR #265 card was passive (only seen if an operator opened `/admin`). A best-effort `recordReminderSchedulerHealthAlert()` now runs from **three** existing daily Vercel crons — `materialize-recurring-breaks` (08:00 UTC), `calendar-reconcile` (09:00) and `calendar-sync` (09:30) — recording a **deduped** ops alert when the heartbeat is **degraded** (`reminder_scheduler_degraded`, warning), **stale** (`reminder_scheduler_stale`, **critical** → emails `OPS_ALERT_EMAILS`) or **missing** (`reminder_scheduler_missing`, critical → emails). No new scheduler/cron/migration; the check never sends reminders and never calls the reminder route.

Three things changed in OPS-01: `stale` escalated from warning to **critical** (only critical severity is emailed, so the old warning meant a dead scheduler stayed silent until the 24h heartbeat TTL expired it into `missing` — up to ~48h); a **degraded** band (31–45 min) was added so a silent cadence regression is no longer reported as `healthy`; and the check moved into a `finally` / gained two more callers so an unrelated failure in one host cron can no longer skip that day's monitoring. Worst-case detection is now **≈22.5h**, down from ~25–48h. The admin card stays the real-time view; operators resolve alerts manually. Full runbook: **docs/08 §"Reminder scheduler alerting + runbook"**.

**Production evidence (2026-08-12, SHA `773dbc7008b5`).** The external scheduler was observed firing at `hone.care` at **23:00:19Z / 23:15:10Z / 23:30:14Z**, all **HTTP 200**, ~15 minutes apart; an unauthenticated probe returned `401`. That is a **proven runtime fact**. Scheduler *ownership* (cron-job.org account owner, backup owner, single-job confirmation, alert recipient) remains an **unverified human fact** — see the checklist in docs/08.
...
| `NEW_CLIENT_WAITLIST_DURABLE_STUDIO_SLUGS` | Optional | ⚠️ **DATED EVIDENCE, NOT A CURRENT READING.** `willow-electrolysis` **WAS** named here — proven for the instants at which durable rows were written, the earliest **2026-08-25T22:27Z** and the latest **2026-09-15**. That is the whole of what the evidence supports. **IT IS NOT A CLAIM ABOUT THE VALUE NOW.** The rows are durable and survive the flag; `lib/booking/new-client-waitlist.ts` re-reads `process.env` on every call, so an operator clearing this variable returns new submissions to the WAIT-01 path **immediately**, while every row already written stays exactly where it is. A persisted row can therefore never prove present configuration. **The value is Sensitive and was NOT read** — not then and not since. To learn the current state, confirm it in the product, not from this table or from row counts. **This entry previously asserted the present tense in both directions**: first that production named no studio, then that it names one. Both over-read their evidence. Setting it is a release decision, never a configuration tweak. **Server-only** comma-separated allowlist of studio **slugs**, matched exactly after trim + lowercase, same semantics as the gate above and read by the same module. Decides whether a waitlisted studio's submissions are COMMITTED TO THE DATABASE (WAIT-02, migration 0185) or delivered as an email (WAIT-01). **Subordinate to the gate**: it is consulted only for a studio the gate has already waitlisted. **Naming a studio in `NEW_CLIENT_WAITLIST_DURABLE_STUDIO_SLUGS` activates nothing unless that studio is also named in `NEW_CLIENT_WAITLIST_STUDIO_SLUGS`.** Unset or empty means every studio stays on the WAIT-01 commit point. Clearing it is the whole kill switch. Also gates the Settings → Waitlist tab. Never prefix `NEXT_PUBLIC_`. **Stage A** additionally forbade production from naming ANY studio, enforced at deploy time, because the public privacy notice did not cover a waitlist prospect. **WAIT-02B Stage B1 shipped that disclosure** (`app/privacy/page.tsx` §§2, 3, 4, 6, 8, 9, plus the collection notice on the public waitlist form), so the blanket prohibition is gone. What remains at deploy time is **report-only**. **Gate 4 is report-only.** **It does not fail the build solely because of `NEW_CLIENT_WAITLIST_DURABLE_STUDIO_SLUGS`.** **Runtime exact-membership is the activation control.** **The production-only configuration report is skipped outside production, while runtime membership still applies.** **A green check is not proof of activation.** That script has no database access and does not read `NEW_CLIENT_WAITLIST_STUDIO_SLUGS`, so it can prove neither that an entry identifies an existing `studios.slug` row nor that the named studio's intake is waitlisted at all; it reports CONFIGURED NORMALISED ENTRIES, an upper bound on what could activate. Confirm activation in the product.

    An earlier Stage-B1 draft *aborted* the build on any entry outside the 1–64 lowercase-alnum-hyphen shape. That shape is what current application writers enforce, **not the domain of `studios.slug`** — migration 0010 adds `slug text` with a UNIQUE constraint and no shape or length check, and its name-based backfill appends a 7-character id suffix without truncating, so a legacy, backfilled or directly-created row can hold a slug outside it that `slugIsListed()` matches exactly. The gate was therefore stricter than the runtime it guards and would have blocked a legitimate activation. The convention check survives only as a **non-blocking WARNING** that asserts nothing about whether a studio exists. Membership stays exact Set equality: no value is ever read as a pattern, and there is no global-enable mode. **Before naming a studio here, confirm the §13 notice process** — material changes are emailed to account holders at least 30 days before taking effect, and `EFFECTIVE_DATE` in `app/privacy/page.tsx` is the one place that records the outcome of that decision. |
