...
| `NEW_CLIENT_WAITLIST_DURABLE_STUDIO_SLUGS` | Optional | **STAGE A: MUST BE EMPTY. It must not be set for any studio, Willow included, until Stage B has shipped the public privacy disclosure for waitlist prospects and served whatever notice that policy requires** — the durable table stores personal information for people the current public notice does not cover (see [03_SECURITY_AND_PRIVACY.md](./03_SECURITY_AND_PRIVACY.md) §8). Setting it is a release decision, never a configuration tweak. **Server-only** comma-separated allowlist, same exact-match semantics as the gate above and read by the same module. Decides whether a waitlisted studio's submissions are COMMITTED TO THE DATABASE (WAIT-02, migration 0185) or delivered as an email (WAIT-01). **Subordinate to the gate**: it is consulted only for a studio the gate has already waitlisted, so listing a studio here while its gate is off enables nothing. Unset or empty means every studio stays on the WAIT-01 commit point — which is what makes the durable code deployable while a studio is live on the old behaviour. Clearing it is the whole kill switch. Also gates the Settings → Waitlist tab. Never prefix `NEXT_PUBLIC_`. |
