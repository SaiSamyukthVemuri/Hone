...
| `manual_fee_charge_attempts` | 0060-0065 | LEGACY fee ledger, read-only history (updated PR #219) | **none since PR #196** (fees unified onto `payment_charge_attempts`; the dead executor `lib/billing/manual-fee-charge.ts` was deleted in PR #218) | one guard SELECT in `lib/billing/manual-fee-eligibility.ts` (blocks prepare while a legacy `active` row exists) | **LEGACY, read-only history** | n/a | n/a | n/a | `stripe_livemode` + `manual_fee_charge_attempts_livemode_false_check` (still intact) | yes | **n/a: no runtime writers; live fees would ride `payment_charge_attempts`** |
...
| 6/7 | No-show / late-cancel fee (updated PR #219) | `app/(app)/calendar/[id]/manual-fee-actions.ts` -> `lib/billing/manual-fee-eligibility.ts` + `lib/billing/session-payment-charge.ts` (unified executor since PR #196) | `payment_charge_attempts` (reason `late_cancellation_fee` / `no_show_fee`) | test only (same gates as flow 2) | same claim RPC + deterministic-key scheme as flow 2 | mechanics yes: inherits receipt/refund/webhook reconciliation (flows 3-5); fee path smoke-closed 2026-06-11 (§16.7) |
...
4. **CI**: `scripts/check-stripe-gates.mjs` (runs in `npm run ci` + GitHub Actions safety gates) pins exact counts: `paymentIntents.create` exactly 1 allowlisted (`lib/billing/session-payment-charge.ts` only; tightened from 2 in PR #218 when the dead legacy executor was deleted), `refunds.create` exactly 1 allowlisted (`lib/billing/payment-refund.ts`), `charges.create` 0, `checkout.sessions` 0, `set_studio_require_card_on_file` 0, the live-mode literal 1. The per-PR gate clone tests (#171-#192 era) were consolidated into the canonical `tests/scripts/check-stripe-gates.test.ts` in PR #218.
...
**RESOLVED (PR #196 unification + PR #218 cleanup; section updated PR #219).** At audit time (PR #192) the ledger was split: `manual_fee_charge_attempts` was still the live runtime for cancellation/no-show fees with no receipt, refund, or reconciliation path, and that was flagged P0. Since then:

- `payment_charge_attempts` (CURRENT, sole runtime ledger): session payments AND cancellation/no-show fees (reasons `late_cancellation_fee` / `no_show_fee`, PR #196, migration 0083); full lifecycle (charge, receipt, refund, webhook reconciliation, ops alerts) applies to all three charge reasons; verified end to end by the §16.7 fee smoke.
- `manual_fee_charge_attempts` (LEGACY, read-only history): zero runtime writers. The dead executor `lib/billing/manual-fee-charge.ts` was deleted in PR #218 and the CI gate tightened to exactly one `paymentIntents.create` call site. The only runtime reference left is a guard SELECT in `lib/billing/manual-fee-eligibility.ts`. Its `livemode_false` CHECK stays in place until the table is retired.
- The 0032 dormant tables (`stripe_charge_attempts`, `stripe_refunds`, etc.) still exist in prod with zero runtime references; dropping or explicitly archiving them remains an open cleanup follow-up (docs/13).

A live fee today would ride the same safety net as a session payment. This section is no longer a live blocker; the remaining live blockers are the human ones in §16 (legal/accounting review, Willow Stripe checklist).
...
> **Update — the §6/§11 "no human-visible ops surface" P0 gap is now resolved.** PR #193 added the admin **Ops alerts** page (`/admin/ops-alerts`, service-role, critical-first, with a `resolved_at` mark-resolved action + critical-alert email via `OPS_ALERT_EMAILS`). PR #285 made every alert **message** redacted-at-write. PR #290 added a dedicated **read-only payment manual-review queue** (`/admin/payments/manual-review`, admin-only) that surfaces, in-app, the two docs/16 §17.7 checks: payment attempts stuck in `pending_stripe` > 60 min, and **unresolved critical payment alerts only** (`session_payment_*` / `payment_intent_*` / `payment_refund_*` / `payment_charge_*` / `charge_refunded_*` / `stripe_webhook_*`). It shows the PaymentIntent id, Hone status, amount, live/test-mode flag, and the redacted message (no client names / payloads / secrets), with a conservative next-step; it is strictly read-only (no resolve/retry/refund button — resolution stays on `/admin/ops-alerts`). So an operator no longer needs raw SQL to find a manual-review payment. **Still NOT live-payment enablement; live payments remain disabled.** (The §11 row 11 "Admin/manual tools: none for payments" entry is superseded by this.) **PR #310** closed a P1 alerting asymmetry: a **failed/non-success** Stripe outcome that Hone cannot persist now fires a **critical** ops alert on both the DB-error (`session_payment_failed_write_failed`, previously stderr-only) and zero-row (`session_payment_failed_write_zero_rows`, promoted from warning) branches — symmetric with the PR #281 success-persistence alerts — so both surface in the critical manual-review queue. Alerting/observability only; no payment behavior change, no new Stripe calls, no migration.
...
>
> **HARDENED by PR #291 (2026-06-30):** `OPS_ALERT_EMAILS` is no longer just "set it to activate" — it is **REQUIRED in production**. The env gate `scripts/check-production-env-gates.mjs` (wired into `npm run build`) **fails the production build** if `OPS_ALERT_EMAILS` does not parse to ≥1 recipient (whitespace-only / comma-only counts as none), so a production deploy can no longer silently ship with the proactive critical-alert email channel disabled. (The original §6 finding text above — "`OPS_ALERT_EMAILS` … never read" — was a PR #153-era state, superseded by #193's `alert-email.ts` and now enforced by #291.) Config verification only: no email is sent at build time, no migration, no runtime/payment change; live payments remain disabled.
...
**Hardened by PR #296 (defense in depth, no migration):** owner-only previously lived ONLY in the two action callers — the shared helper `refundPaymentChargeAttempt` re-checked studio scope + live-mode but not `role === "owner"`, so a future caller reaching the helper without the action gate could have bypassed owner-only. PR #296 adds a helper-level owner re-check (reads the actor's `practitioners.role` scoped to `(practitionerId, studioId)`, returns the existing `not_authorized` outcome) **before the claim UPDATE and the single `refunds.create`**, plus `logInternal("payment_refund_denied_not_owner", …)` safe-ID-only logging on the action denial paths. The `refunds.create` count stays exactly one; no schema/RLS change; live payments remain disabled.
...
> **Health-inspection note (PR #205, 2026-06-12):** record keeping (sterile items, disinfectants, exposure incidents, client procedure records + probe lot capture) has STARTED in Hone, built from Chloe's BodySafe sample forms, PR #206 added an append-only, trigger-written audit trail with edit support, and PR #213 added probe lot traceability (sterile items to charted usage). Still needs Chloe/public-health review before operational reliance. It still needs Chloe/public-health review before being relied on operationally; it is not a legal compliance guarantee. Unrelated to payments; all live-payment blockers above unchanged.

> **PR #218 (2026-06-12):** the dead legacy manual-fee executor (`lib/billing/manual-fee-charge.ts`) is removed and the Stripe gate tightened to exactly ONE `paymentIntents.create` call site (the unified `session-payment-charge.ts` executor). The legacy `manual_fee_charge_attempts` table remains read-only history. All live-payment blockers above are unchanged.
