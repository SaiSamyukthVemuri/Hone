...
- **Action:** `app/(app)/clients/[id]/sessions/[sessionId]/payment-actions.ts:refundPaymentChargeAttemptAction` (session payments) and `app/(app)/calendar/[id]/manual-fee-actions.ts:refundFeeAttemptAction` (no-show / late-cancel fees). Accepts `attempt_id` + optional `internal_note` + `session_id` / `client_id` for revalidate; never accepts a browser-supplied amount.
- **Authorization — studio-owner-only (PR #201; hardened PR #296).** Refund **initiation is owner-only**: both action callers require `practitioner.role === "owner"` (active studio member) before any Stripe/DB work and return "Only the studio owner can issue a refund." for non-owners (a denied attempt is `logInternal`-logged with safe IDs only — `payment_refund_denied_not_owner`). **Charging is intentionally member-allowed; refunds (money out) are owner-only.** PR #296 adds a **defense-in-depth owner re-check inside the helper** `refundPaymentChargeAttempt` (reads the actor's `practitioners.role` scoped to the studio, before the claim UPDATE and the `refunds.create`), so a future caller that reaches the helper without the action gate still cannot refund as a non-owner. No migration; no platform-admin path (owner = the per-studio `practitioners.role`, not `ADMIN_EMAILS`).
...
### Failure-persistence alert symmetry (PR #310)

Pre-live review found a **P1 asymmetry** in `writeFailedOutcome`: when Stripe returned a **failed/non-success** outcome and Hone could not persist the local `failed` state, the **DB-error** branch logged to stderr **only (no ops alert)** and the **zero-row** branch raised a **warning** — both invisible to the critical-only manual-review queue, unlike the succeeded-outcome path (PR #281, critical on both). PR #310 makes them **symmetric**: the DB-error branch now raises a **critical** `session_payment_failed_write_failed`, and the zero-row branch is **promoted warning → critical** `session_payment_failed_write_zero_rows`. Both carry the same safe context as the success alerts — `studioId`, `clientId` (opaque ids), `stripePaymentIntentId`, `route`, and `safeDetails` of `attempt_id` / `attempted_status:"failed"` / `stripe_status` / `db_code` — **no card details, client names/emails/phones, health/treatment notes, or raw Stripe payload**. Because both events start with `session_payment_`, they surface **automatically** in the `/admin/payments/manual-review` critical queue (existing prefix filter) and page operators via `OPS_ALERT_EMAILS` — **no manual-review page change**. **Success/failure persistence-failure alerting is now symmetric.** **Alerting/observability only — no payment behavior changed, no new Stripe calls, no live-payment enablement, no migration.** Pinned by `tests/lib/billing/payment-failure-persistence.test.ts` (+ the promoted-severity assertions in `payment-outcome-zero-row.test.ts`).
...
The **in-app, payment-attempt-centric surface** of the §282 reconciliation checks. PR #281 leaves a Stripe-succeeded-but-not-persisted charge in `payment_charge_attempts.status='pending_stripe'` and records a **critical** ops alert (`session_payment_succeeded_write_failed` / `_zero_rows`; the retrieve-failed / stale-pending / unknown-error paths record `session_payment_needs_manual_review`); refunds do the same (`payment_refund_succeeded_write_failed` / `_stripe_unknown_outcome` / `_stripe_returned_null`). There is **no `needs_manual_review` DB status** — the row stays `pending_stripe`. PR #290 adds a **read-only admin page** `/admin/payments/manual-review` (admin-only via the existing `ADMIN_EMAILS`/`isAdmin` gate + a defensive page re-check; service-role read; `force-dynamic`) with two derived sections that mirror docs/16 §17.7 queries (1) and (3)/(6): (1) **stuck attempts** — `payment_charge_attempts` where `status='pending_stripe'` and `updated_at < now()-60min`; (2) **unresolved critical payment alerts** — `ops_alerts` where `severity='critical'`, `resolved_at IS NULL`, and the event is a payment manual-review event (`session_payment_*` / `payment_intent_*` / `payment_refund_*` / `payment_charge_*` / `charge_refunded_*` / `stripe_webhook_*`). **Critical-only** by design — warning-level reconciliation alerts (no_match, `*_reconcile_zero_rows`, livemode-event-ignored) and card-on-file setup failures stay on the full `/admin/ops-alerts` list. It displays only safe fields (created/updated, studio name+id, `client_id`/`session_id`/`appointment_id` **ids only — no client names**, amount/currency, Hone `status`, `charge_reason`, `failure_code`, the non-secret `stripe_payment_intent_id`, a **live/test-mode** flag, the alert event/severity, and the **redacted** alert message) plus a conservative next-step ("review the PaymentIntent in Stripe, compare with the Hone attempt, do **not** retry/refund blindly, follow docs/16 §17"). **It is strictly READ-ONLY: no resolve/retry/refund/repair button, no payment-attempt mutation, no Stripe API call.** Resolution stays on the existing `/admin/ops-alerts` resolve action (the page links there). A failed read surfaces loudly (generic error) rather than a misleading empty queue. **No migration, no charge/refund/webhook behavior change, Stripe gates unchanged (1 `paymentIntents.create` / 1 `refunds.create` / 0 `charges.create` / 0 `checkout.sessions`), live payments remain disabled.** Pinned by `tests/app/admin/payment-manual-review.test.ts`.

### Critical ops-alert delivery verification (PR #291)

Operations reliability hardening before live payments. Payment failures that need an operator — Stripe-succeeded-but-not-persisted (`session_payment_succeeded_write_*`), refund write failures, webhook terminal-state/livemode/metadata mismatches, disputes — record **critical** `ops_alerts` and email the recipients in `OPS_ALERT_EMAILS` via `lib/ops/alert-email.ts` (after the durable row, never throws). If `OPS_ALERT_EMAILS` is unset/empty that email is a silent no-op and the alert lives only as a DB row + `/admin/ops-alerts` — operators might not see a payment failure quickly. **PR #291 makes `OPS_ALERT_EMAILS` REQUIRED in production:** the env gate `scripts/check-production-env-gates.mjs` (wired into `npm run build`) fails the production build if it does not parse to ≥1 recipient (whitespace-only / comma-only counts as none); enforced only when `VERCEL_ENV === "production"` (local/CI/preview SKIP), names-only output. **Config verification only — no migration, no runtime change to the alert/payment path, no email sent at build time, no new provider, Stripe gates unchanged (1/1/0/0), live payments remain disabled.** The read-only payment manual-review queue (PR #290) is a separate surface and is unaffected. Pinned by `tests/scripts/check-production-env-gates.test.ts`.
