...
4. **DB CHECK replacement** — drop/replace `payment_charge_attempts_livemode_false_check` (`check (stripe_livemode = false)`, migration 0073) via a **migration**. Per the 0073 header this is the named structural dormancy guard; relaxing it is deliberate.
5. **Claim-RPC mirror relaxation** — the `claim_session_payment_charge_attempt` RPC (migration 0075) returns `not_ready` for any `stripe_livemode <> false` row; relax/replace that body (paired migration).
