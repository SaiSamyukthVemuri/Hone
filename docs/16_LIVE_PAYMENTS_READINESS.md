...
> **[HISTORICAL — Status (PR #201, 2026-06-12); superseded, see banner above]:** docs/18 §16 is the current gate-preparation state. Live receipt template readiness shipped (live branch unreachable; wording needs legal/accounting review); refunds are now owner-only across all charge reasons; stale pending_stripe recovery verified already-shipped; payment audit trail decided as ledger-row-based. Live payments remain disabled; controlled enablement is future PR #202 and is blocked on the PR #196 late-cancel smoke, legal/accounting review, and Willow Stripe live-account readiness.

**[HISTORICAL — 2026-06-10] Status: NOT READY FOR LIVE PAYMENTS** *(superseded 2026-07-08 — supervised live session payments are now live for approved studios; see the banner above)* (original review PR #168, 2026-06-08; reaffirmed after PRs #170-#187, 2026-06-10; **fully re-audited in [docs/18_LIVE_PAYMENTS_AUDIT.md](./18_LIVE_PAYMENTS_AUDIT.md), PR #192, 2026-06-10; both are retained as the historical review + checklists**). Test-mode session payments are built end-to-end (prepare, charge, receipt, refund, webhook reconciliation, billing UX) and practitioner-verified; the live paths have since been enabled for approved studios. At the 2026-06-10 date live was still blocked and fees not active. Several §5 blockers carry resolution notes below; the open ones are: legal review of card-authorization wording, tax/HST decision, statement descriptor review, off-session SetupIntent confirmation review, live runbook, dispute-response runbook, Willow live Stripe onboarding, supervised first live charge, late cancellation/no-show fee charging on `payment_charge_attempts`, and `manual_fee_charge_attempts` unification or retirement.
...
- **Operators already have full visibility** via the admin **Ops alerts** page (`/admin/ops-alerts`): unresolved-critical-first, with event, message, PaymentIntent id, and redacted details; critical alerts also email `OPS_ALERT_EMAILS`. No new dashboard is required for reconciliation. **(PR #291) `OPS_ALERT_EMAILS` is now REQUIRED in production** — the env gate (`scripts/check-production-env-gates.mjs`, wired into `npm run build`) fails the production build if it does not list ≥1 recipient, so the proactive critical-alert email channel cannot silently ship disabled before live payments. The payment manual-review queue (PR #290) remains the read-only in-app surface; this gate is delivery-config verification only (no runtime/payment change, live payments remain disabled).
...
**This subsection is the single source of truth for HOW live payments get turned on.** It consolidates what was previously scattered across §6 / §11 / §12.13 / §17. **PR #297 changes NOTHING operational — it documents the sequence and adds a CI safety-lock (`tests/lib/billing/live-mode-disabled.test.ts`) that fails if any gate below is relaxed without a deliberate, reviewed change.** Live payments remain **DISABLED** today.

**Flipping the env flag alone is NOT enough.** `STRIPE_ALLOW_LIVE_MODE=true` only re-arms the key gate; the **runtime dormancy guards, the DB CHECK, the claim-RPC mirror, and the webhook live-event ignore all independently keep live mode off** and must each be relaxed first, in their own deliberate PRs. The env flip is the **LAST** step.

Already SHIPPED backstops (do not re-do): payment success persistence + `needs_manual_review` (**PR #281**); read-only payment manual-review queue `/admin/payments/manual-review` (**PR #290**); `OPS_ALERT_EMAILS` **required in production** so critical alerts email operators (**PR #291**); refunds **owner-only**, hardened with a helper-level re-check (**PR #201 + #296**).

**Enable in this exact order — each numbered step is its own reviewed change; do not reorder:**

1. **Business / legal go/no-go.** Receipt vs invoice + tax wording, refund + cancellation policy text, card-on-file authorization wording, statement descriptor, privacy/Terms updates, retention + dispute process — all reviewed and approved (legal/accounting). The PR #196 late-cancel smoke closed.
2. **Live Stripe account readiness.** The studio's **live** connected account: onboarding complete, `charges_enabled = true`, **`payouts_enabled = true`**, currency CAD, `application_fee_bps` decision made.
3. **Live webhook endpoint readiness.** A **live connected-account** webhook endpoint configured with `account.updated`, `payment_intent.succeeded`, `payment_intent.payment_failed`, `charge.refunded`, `charge.dispute.created`, `setup_intent.succeeded`; its live `whsec_` captured.
4. **DB CHECK replacement** — ✅ **DONE in migration `0101` (PR #322 / PR A).** Dropped `payment_charge_attempts_livemode_false_check` (`check (stripe_livemode = false)`, migration 0073) and replaced it with the narrower structural `payment_charge_attempts_live_requires_account_check` (`check (stripe_livemode = false or stripe_account_id is not null)`) — a live row must carry a connected account; test-mode rows are unconstrained; `stripe_livemode` default stays `false`. Inert (runtime + env still block live charging; no live row is created). The legacy `manual_fee_charge_attempts_livemode_false_check` (0065) is deliberately untouched.
5. **Claim-RPC mirror relaxation** — ✅ **DONE in the same migration `0101` (PR #322).** `claim_session_payment_charge_attempt` (body from migration 0083) had its `if v_row.stripe_livemode <> false then return not_ready` block removed so it can claim live rows once they exist; every other guard (reason / status / authorization / PI-id / idempotency / conditional claim) and the `service_role`-only grant are unchanged.
6. **Charge runtime guard relaxation** — ✅ **DONE in PR #323.** `session-payment-charge.ts`: removed the `inferStripeLivemode() === true` early return; the row + card/settings/customer lineage guards now compare against `inferStripeLivemode()` (mode-consistency, not hardcoded false); prepare-insert writes `stripe_livemode: inferStripeLivemode()`; `hone_environment` metadata is dynamic.
7. **Refund runtime guard relaxation** — ✅ **DONE in PR #323.** `payment-refund.ts`: removed the env early return; the row guard + `.eq("stripe_livemode", livemode)` claim predicate + `hone_environment` metadata are env-gated.
8. **Receipt runtime / wording relaxation** — ✅ **DONE in PR #323 (guard) + ✅ WORDING LEGAL-APPROVED (2026-07-04).** `payment-receipt.ts`: the `stripe_livemode !== inferStripeLivemode()` guard makes the sender live-CAPABLE and passes the row's real mode to the template. The live receipt now carries the **lawyer-approved wording** (no tax-invoice claim, no refund promise, Hone not merchant of record, "Payment method: Card ending in {last4}"). No live receipt is sent while the env is test (no live row exists).

   **Copy status (updated 2026-07-02, internal-copy fast-follow):** the ~52 internal practitioner/fee-card "test charge"/"test-mode" labels (deferred from #323) are now NEUTRALIZED so they read correctly in live; the settings test-mode banner + #323 mode-gated portal copy remain the explicit while-live-off warning.

   **✅ LEGAL/ACCOUNTING COPY SIGN-OFF COMPLETE (2026-07-04):** the lawyer-approved live **receipt** wording (`lib/email/templates/payment-receipt.ts` live branch), the client-facing **card-on-file authorization** wording, and the **replace-card** wording (`app/portal/PortalCardOnFileCard.tsx` + `PortalPaymentMethodForm.tsx`) are now in place, with test-mode warnings still mode-gated while live is off. **This clears the last remaining COPY precondition for the #324 env flip.** The remaining #324 preconditions are Willow's live Connect onboarding + the §17.14 human go/no-go checklist — not copy.
9. **Webhook live-event relaxation** — ✅ **DONE in PR #323.** `shouldIgnoreLiveModeEvent` now MODE-MATCHES: it processes events whose `event.livemode` equals `inferStripeLivemode()` and safely ignores mismatched-mode events (warning `stripe_webhook_livemode_event_ignored`); the per-handler row backstops compare against `inferStripeLivemode()`.
10. **Card-authorization livemode scoping** — ✅ **Already dynamic (no #323 change needed).** `current-card-authorization.ts` + `refresh-card-authorization-pointer.ts` already scope lookups to `inferStripeLivemode()`; live card-on-file rows resolve once they exist.
...
The final pre-live review found a **P0 gap**: the repo requires a specific production migration max (the repo-derived expected max — currently **0105**; verify-production derives it, so it tracks the repo), but the review could **not independently confirm remote production state**. `scripts/verify-production.mjs` closes that gap — an **operator-run, READ-ONLY** health check that proves remote production matches the repo's required state and **reports** the current live/test payment posture. Production is now **live-capable** (live billing was proven on a controlled test studio); run it before **Willow's own live onboarding**, before broadening sensitive-data use, or any time remote production state must be independently confirmed. It is **not** a CI gate (CI has no production link, by design) and **not** a live-payment enablement script.
...
**What it reports (observations — INFO lines, never pass/fail gates, counts/booleans only):** the runtime Stripe mode of the loaded env-file (key SHAPE `sk_live_`/`sk_test_` + `STRIPE_ALLOW_LIVE_MODE`, never the secret value); `studio_payment_settings` row counts by mode (live / test / null-mode placeholder); live connected-account counts with `charges_enabled` / `payouts_enabled` / `account_status=enabled` (**counts only — no account ids**); active card counts by mode; `payment_charge_attempts` counts by mode + status; live webhook-event totals with errored / unprocessed counts; and unresolved **warning** payment ops-alert count. These are for operator visibility and do **not** affect the PASS/FAIL/INCOMPLETE summary or exit code.
...
- **Vercel PRODUCTION env presence** — `OPS_ALERT_EMAILS` set; Upstash vars set; **`STRIPE_ALLOW_LIVE_MODE`** and the `STRIPE_SECRET_KEY` mode match the intended deployment posture (production is **live-capable**; the flag is `true` alongside the `sk_live_` key). (Vercel env vars are write-only; the proof is the last green production deploy passing `check-production-env-gates.mjs` plus a dashboard glance — not a value read.)
- **Stripe dashboard** — mode / keys / connected-account state for the studio being operated.
- **storage.objects policies** — `storage.objects` policies are owned by `supabase_storage_admin` and are **not introspectable** from the linked query role, so this stays manual. In the Supabase dashboard (Storage → policies) confirm the hardened end state: **objects are service-role-only** — there is **no `authenticated`/`anon` policy granting access to `treatment-images`** (migration 0093 dropped the 0092 authenticated select/insert policies and added none, so the `authenticated` role is deny-by-default on `storage.objects`), and **no foreign-bucket policy OR-combines onto `storage.objects`** (a permissive policy scoped to another bucket must not widen access to the `treatment-images` objects). All object access is server-mediated (service-role upload + short-TTL `createSignedUrl` after a studio+client ownership check + the `validateTreatmentImagePath` binding); the private bucket + these are the isolation guarantees. The db-integration lane pins the service-role-only end state on the real migrated DB (`tests/db/treatment-image-hardening.db.test.ts`), but the live prod policy list itself can only be eyeballed here.
...
This is a **read-only health check**, not an enablement step. A studio's live enablement still follows the operator sequence (§17.14 detail; §17.15 the Willow/Chloe condensation).

### Current production state (post-live-proof) — the truth this runbook now describes

As of the app-wide payment-messaging work (PRs #332–#339) and the controlled live test:

- **Production is LIVE-CAPABLE.** `STRIPE_ALLOW_LIVE_MODE=true` with a `sk_live_` key in Vercel Production; prod DB migration max = the current repo-derived expected max (derive it — see the banner at the top of this doc; canonical in current-state.md / migration-ledger.md); the runtime read-out is the Dashboard "Live payments" stat.
...
- **Out of launch scope** (do not present as available/automatic): partial payments, deposits, packages/gift cards/subscriptions, application fees, a second studio, broad self-serve rollout. **Manual no-show / late-cancellation fees remain a supervised HOLD** — fee amounts are configuration only; a practitioner runs each fee explicitly and only if explicitly approved.

### 17.14 Controlled live-payment env flip + first live charge — operator runbook (PR C)

> **READ THIS FIRST.** **Live payments are still OFF.** This section is DOCUMENTATION ONLY. **This PR does not flip the env, does not touch Stripe, does not create any webhook endpoint, does not run any payment, and changes no code behavior or copy.** The actual env flip and the first live charge are **separate operator actions**, taken by hand **after** this runbook merges AND every go-condition below is met. **Legal/accounting sign-off on the live receipt + client-facing card-authorization wording remains MANDATORY before the env flip; if legal changes any wording, a separate copy-only PR (updating `lib/email/templates/payment-receipt.ts` + the portal card-authorization strings) must merge before the flip.** Scope is **one supervised studio (Willow / Chloe) and one small supervised charge** — nothing else.

**Why the flip is more than a flag.** Stripe Connect accounts are **mode-specific**: Willow's current `acct_…` is a **test** account. A live account requires a **fresh live Express onboarding**, which can only happen **after** the deployment is in live mode (onboarding calls `getStripe()` with the live key). All runtime lookups scope by `stripe_livemode = inferStripeLivemode()`, so once the deployment is live, the test settings/cards/customers go **dormant** (not deleted) and Willow must onboard fresh in live. Two ordering traps: (a) `NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY` is **build-time inlined** → a **fresh build** is required, not a cache-reuse redeploy; (b) `assertStripeKeyAllowed` (`lib/stripe/server.ts`) **throws** if `sk_live_` is set while `STRIPE_ALLOW_LIVE_MODE !== "true"` → **never** deploy the live key with the flag unset.

#### 17.14.1 Final go/no-go checklist (ALL must be ✅ before the first charge)
...
4. [ ] Deployment healthy: `hone.care` loads, **no `getStripe`/`STRIPE_SECRET_KEY` throw** in logs, `node scripts/check-stripe-gates.mjs` = **15 PASS**, `node --env-file=.env.local scripts/verify-production.mjs` = **PRODUCTION VERIFIED ✓** (this asserts remote migration max == the repo-derived expected max — do not hardcode a number here).
...
8. [ ] Read-only SQL clean (§17.14.8): `payment_charge_attempts where stripe_livemode=true` count = **0**; unresolved critical ops_alerts = **0**.
9. [ ] Rollback + kill-switch (§17.14.10) rehearsed; operator watching all surfaces (§17.14.9); amount **small**; card operator-controlled.

**If ANY line is unchecked → STOP (see §17.14.11 no-go conditions).**

#### 17.14.2 Legal/accounting sign-off requirements (MANDATORY before the flip)

> **✅ SATISFIED (2026-07-04).** The lawyer approved the live receipt, card-on-file authorization, and replace-card wording, and the approved text is now implemented (copy-only PR + one display-only `last4` read). The requirements below are retained for the record; the actual strings in the code are the source of truth.

- **Live receipt wording** — the live branch of `lib/email/templates/payment-receipt.ts` (currently DRAFT, marked "PENDING legal/accounting review"). Legal/accounting must approve the exact strings: lead "Receipt for card payment processed by {studio}."; no-tax "No tax calculation is included on this receipt unless separately stated by the studio."; support "For questions about this payment or refund eligibility, contact the studio."; subject "Receipt from {studio}: {reason} {amount}". They confirm: **no** tax-receipt / official-invoice / charitable-receipt claim, **no** refund-policy promise, accurate for a CA aesthetics service.
- **Client-facing card-authorization wording** — the #323 mode-gated live portal strings: `PortalCardOnFileCard` "Your card on file may be charged by the studio for authorized fees per the policy you agreed to."; `PortalPaymentMethodForm` "Your current card will be replaced after the new card is saved. It may be charged for authorized fees per the studio's policy." They confirm the authorization/consent language is accurate and enforceable.
- **If approved AS-IS → no code change.** **If reworded → a separate copy-only PR** (`payment-receipt.ts` template + the two portal strings) must merge **before** the env flip. This runbook PR does **not** change that copy.

#### 17.14.3 Willow live Stripe Connect readiness (LIVE account, Stripe live Dashboard → synced to `studio_payment_settings`)

- [ ] `charges_enabled = true`
- [ ] `payouts_enabled = true`
- [ ] `details_submitted = true`
- [ ] `requirements.currently_due` **empty** (and `eventually_due` reviewed)
- [ ] **CAD** external bank account added + **verified**
- [ ] **Statement descriptor** reviewed on the connected account (the app sets none in code — `session-payment-charge.ts` sets no `statement_descriptor`; the account default is what the client sees)
- [ ] **Dispute email / recovery / 2FA** reviewed (dispute-notification email set, 2FA on, Radar reviewed)

#### 17.14.4 Live webhook endpoint

- **URL:** `https://hone.care/api/stripe/webhook` (the existing route — no code change).
- **Type:** a **Connect** webhook endpoint in the Stripe **live** Dashboard with **"Listen to events on Connected accounts" enabled** (the handler reads `event.account`).
- **Signing secret** → becomes `STRIPE_WEBHOOK_SECRET` (§17.14.5).
- **Required events** (the handler acts on exactly these; all others are recorded as `ignoredInPhase1`):
  - `account.updated`
  - `capability.updated`
...
#### 17.14.5 Exact Vercel Production env vars

| Var | Value | Read at | Notes |
|---|---|---|---|
| `STRIPE_ALLOW_LIVE_MODE` | `true` | runtime | the master switch |
| `STRIPE_SECRET_KEY` | `sk_live_…` | runtime | |
| `NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY` | `pk_live_…` | **build** | requires a fresh build |
| `STRIPE_WEBHOOK_SECRET` | `whsec_…` (the **live** endpoint's secret) | runtime | |
| `STRIPE_CONNECT_COUNTRY` | confirm `CA` | runtime | already defaults to `CA` |

(`OPS_ALERT_EMAILS`, `RESEND_API_KEY`, `UPSTASH_REDIS_REST_URL/TOKEN` must already be set — alerts, receipts, heartbeat — and are unchanged.)

#### 17.14.6 Exact env-flip order

1. **Legal/accounting sign-off** complete (§17.14.2). If reworded, the copy-only PR is merged.
2. **Create the live Connect webhook endpoint** (§17.14.4) → obtain `whsec_`.
3. In Vercel **Production**, set **all four** live vars (§17.14.5) **in one batch, before any redeploy**; confirm `STRIPE_CONNECT_COUNTRY=CA`.
4. Trigger **one fresh production build/deploy** (full build, so `pk_live_` is inlined).
5. **Verify health** (go/no-go items 3–4).
6. **NEVER deploy `sk_live_` while `STRIPE_ALLOW_LIVE_MODE` is false/unset** — that deploy hard-fails every Stripe route (`assertStripeKeyAllowed` throws).

#### 17.14.7 First live charge procedure (all in-app, supervised)

1. **Willow live onboarding** — Settings → Payments → complete live Connect onboarding.
2. **Confirm the live settings row** — `studio_payment_settings` has `stripe_livemode=true`, `stripe_account_status='enabled'`, `charges_enabled=true` (synced via the `account.updated` webhook).
3. **Supervised client signs the card-authorization consent** (recommend Chloe's own real card).
4. **Supervised client adds a live card** on file via the portal → `setup_intent.succeeded` → `client_payment_methods` (`stripe_livemode=true`, `status=active`).
5. **Prepare one small charge** — session payment or a small manual fee, CAD, small amount.
6. **Run charge** (two-click confirm) → direct charge on the connected account (`confirm:true`, off-session, **0 application fee**, no `statement_descriptor` in code) → "Charge succeeded."
7. **Verify webhook reconciliation** — `payment_intent.succeeded` reconciles the row idempotently (already `succeeded` from the synchronous path).
8. **Send the receipt** — **only if** the live receipt wording is legal-approved (§17.14.2).
9. **(Recommended close-out) Refund through the app** to return the funds and validate the live refund lifecycle (`charge.refunded` → `refund_status='succeeded'`). This is the existing refund path — not a new charge.

#### 17.14.8 Read-only SQL checks (via `supabase db query --linked`, SELECT-only)

**Before (post-flip health + pre-charge):**
```
select max(version) from supabase_migrations.schema_migrations;                          -- expect 0101
select count(*) from payment_charge_attempts where stripe_livemode = true;               -- expect 0
select count(*) from ops_alerts where severity='critical' and resolved_at is null;       -- expect 0
-- after Willow onboards live:
select stripe_livemode, stripe_account_status, charges_enabled, payouts_enabled
  from studio_payment_settings where studio_id = '<willow_studio_id>';                    -- live row, enabled, both true
-- after the client adds a live card:
select count(*) from client_payment_methods
  where stripe_livemode = true and status='active' and studio_id='<willow_studio_id>';    -- expect 1
```
**After the charge:**
```
select id, status, stripe_livemode, amount_cents, currency, refund_status
  from payment_charge_attempts where stripe_livemode = true
  order by created_at desc limit 1;                                                       -- one row, succeeded, livemode true
select count(*) from ops_alerts where severity='critical' and resolved_at is null;       -- expect 0
```
Also run `node --env-file=.env.local scripts/verify-production.mjs` (read-only) — expect **PRODUCTION VERIFIED ✓** (which asserts remote migration max == the repo-derived expected max — never hardcode it here).

#### 17.14.9 Operator surfaces to watch (during + after)

- **Stripe live Dashboard** — the PaymentIntent + Charge on Willow's account (small CAD amount, 0 application fee).
- **Connect account health** — Willow's account status, requirements, payouts.
- **Webhook delivery** — Stripe → Webhooks: the live endpoint delivering 200 (no 4xx/5xx retries).
- **Vercel production logs** — no `getStripe` / `STRIPE_SECRET_KEY` / `sync_studio_account_status` errors.
- **Ops alerts** — `/admin/ops-alerts` + the read-only SQL count (critical/warning).
- **Manual-review queue** — `/admin/payments/manual-review` (any `needs_manual_review`).
- **Practitioner UI** — the "Charge succeeded" panel.
- **Resend** — delivery of the (approved) live receipt.

#### 17.14.10 Rollback / kill switch

- **Kill-switch (fastest):** in Vercel Production **unset/flip `STRIPE_ALLOW_LIVE_MODE`** and **redeploy** → `getStripe()` fails closed on the live key → no further live charges (~1–3 min redeploy window; acceptable under supervision).
- **Bad charge:** a live charge is **real money** — **refund it via the app (while still live) or in the Stripe Dashboard** *before* rolling back keys. Rollback does **not** un-charge.
- **Full revert to test:** restore `STRIPE_SECRET_KEY=sk_test_`, `NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY=pk_test_`, the **test** `STRIPE_WEBHOOK_SECRET`, unset `STRIPE_ALLOW_LIVE_MODE` → **fresh redeploy** → test mode restored.
- **Live rows stay dormant, not deleted** — mode-scoping (`stripe_livemode`) hides them from the test deployment; nothing is destroyed.

#### 17.14.11 Exact no-go conditions (STOP if ANY is true)
...
- Deployment unhealthy: `getStripe` throw, `check-stripe-gates` ≠ 15 PASS, `verify-production` not `PRODUCTION VERIFIED ✓`, or remote migration max ≠ the repo-derived expected max (verify-production reports this).
...
- Rollback/kill-switch not rehearsed, or no operator watching the surfaces.
- Anything other than **one small charge** on **one operator-controlled card** at **one studio (Willow)**.

#### 17.14.12 Explicitly out of scope

Deposits · packages · gift cards · subscriptions · application fees · `checkout.sessions` · Stripe Terminal · any second studio · any broad or self-serve rollout · more than one small supervised charge. Each is a separate, future, owner-approved decision — none is enabled by the flip.
...
This is the concise operator condensation of §17.14; §17.14 remains the authoritative detail. Merged-state prerequisites already satisfied: ✅ legal/accounting copy approved + merged · ✅ production DB at migration 0105 (0103 Connect provisioning + 0104 one-active-card-per-mode APPLIED + proven; 0105 mode-scoped attempt uniqueness — apply migration-first after its PR merges) · ✅ runtime/webhook/receipt live-capable · ✅ `check-stripe-gates` 15 PASS · live payments OFF. **Go order: §1 → §2 → §3 → §4 → §5 (all green) → §6 → §7 (→ §8), with §9 rehearsed and §10 clear before charging.**
...
- [ ] `node --env-file=.env.local scripts/verify-production.mjs` = PRODUCTION VERIFIED ✓ (asserts remote migration max == the repo-derived expected max — do not pin a number)
...
- [ ] Unhealthy deploy: `getStripe` throw · gates ≠ 15 PASS · `verify-production` not ✓ · verify-production reports remote migration max ≠ the repo-derived expected max
- [ ] Any unresolved critical ops alert
- [ ] No live `studio_payment_settings` row, or no live active card on file
