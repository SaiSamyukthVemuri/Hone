...
6. **Charge runtime guard relaxation** — `lib/billing/session-payment-charge.ts` (the `inferStripeLivemode() === true` and row-level `stripe_livemode !== false` `live_mode_blocked` returns, + the card/settings/customer lineage scoping).
7. **Refund runtime guard relaxation** — `lib/billing/payment-refund.ts` (the env + row `live_mode_blocked` returns + the `.eq("stripe_livemode", false)` claim predicate).
8. **Receipt runtime / wording relaxation** — `lib/billing/payment-receipt.ts` (the `stripe_livemode !== false` refusal) + the hardcoded `livemode: false` email template, with the legal-approved live wording from step 1.
