...
8. **Receipt runtime / wording relaxation** — ✅ **DONE in PR #323 (guard) — WORDING GATED on legal sign-off.** `payment-receipt.ts`: the `stripe_livemode !== inferStripeLivemode()` guard makes the sender live-CAPABLE and passes the row's real mode to the template (which already has a live branch). **#324 MUST NOT proceed until the live receipt wording (`lib/email/templates/payment-receipt.ts` live branch) AND the client-facing card-authorization copy (portal, PR #323) have legal/accounting approval.** No live receipt is sent in #323 (env is test → no live row exists).

   **Copy status (updated 2026-07-02, internal-copy fast-follow):** the ~52 internal practitioner/fee-card "test charge"/"test-mode" labels (deferred from #323) are now NEUTRALIZED so they read correctly in live; the settings test-mode banner + #323 mode-gated portal copy remain the explicit while-live-off warning. **The ONLY remaining copy precondition for #324 is legal/accounting sign-off on the CLIENT-FACING live receipt + card-authorization wording** — no internal-copy work remains.
...
1. [ ] Legal/accounting **written** sign-off on the live **receipt** wording AND the client-facing **card-authorization** wording (§17.14.2). If reworded, the copy-only PR is merged.
