...
// uses the server action in ./actions.ts. v1 is list-only: no
// realtime, no per-row mark-read, no filtering, no preferences.
    ...
    <div className="flex flex-col gap-6">
      <header className="flex flex-wrap items-baseline justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-neutral-500">
            New bookings, cancellations, and reschedules across {studio.name}.
          </p>
        </div>
        {unreadCount > 0 && (
      ...
      {notifications.length === 0 ? (
        <div className="rounded-lg border border-dashed border-neutral-300 bg-neutral-50 px-5 py-12 text-center text-sm text-neutral-500 dark:border-neutral-700 dark:bg-neutral-900">
          ...
          from public clients will show up here.
        </div>
      ) : (
