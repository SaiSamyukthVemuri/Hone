...
// inside getStudioBySlug. No client, no appointment, no intake, no notification
// row, and NOTHING in `public.waitlist` — that table is the Hone marketing
// early-access list (global email uniqueness, no studio ownership), a different
// product concept. Durable storage is V2's job under a separately authorised
// migration.
//
...
// the posted slug — is both the rate-limit scope and the idempotency-key tenant
// component. A forged slug can only ever select WHICH studio is resolved; it
// can never become the identity used downstream.
//
// PII. Name / email / phone belong in the two emails and NOWHERE else. No log
// line, error message or event authored here carries them — nor the idempotency
// key or canonical payload, which are derived from them.
// ===========================================================================
...
export async function submitNewClientBookingWaitlistAction(
  ...
  //    The sender derives the idempotency key from the SERVER-RESOLVED studio
  //    id plus the exact payload it transmits, so neither the tenant scope nor
  //    the bytes can drift from what is actually sent. Both email builders are
  //    pure functions of the submission, which is what makes an identical
  ...
  //    its own namespace and the same tenant scope. Any outcome, including a
  //    throw, leaves the result successful: the studio already holds the
  //    request and a failed courtesy note must never erase the record.
  try {
