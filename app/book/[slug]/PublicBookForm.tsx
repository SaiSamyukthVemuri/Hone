...
type Props = {
  ...
  // Defaults to FALSE so every pre-existing render path, and any call site
  // that does not pass it, is byte-for-byte the previous behaviour.
...
export function PublicBookForm({
  ...
  // no slot list, and no slot fetch (the effect above short-circuits). They
  // are never walked through a booking flow only to be refused at the end.
  // Placed BEFORE the no-consultation-service branch: a waitlisted studio's
  // service catalogue is irrelevant because nothing here books anything.
