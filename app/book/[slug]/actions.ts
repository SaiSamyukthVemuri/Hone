...
export async function publicBookAppointmentAction(formData: FormData): Promise<PublicBookResult> {
  ...
  // hand-forged form can all reach this action with client_type=new after
  // the studio has been put into waitlist mode.
  //
  // Enablement is derived from the SERVER-RESOLVED studio slug (never a
  // browser-supplied value), and the refusal happens HERE: before the
  // readiness read, before the service read, before the slot re-verification,
  // and therefore before any client insert, appointment command, intake
  // creation, booking notification, or booking-conversion analytics. No
  // business state can be mutated on the refused path.
  //
