...
// ===========================================================================
//
// The ONE reserved slug this lane enables. It is named literally in
// e2e/helpers/local-env.ts (NEW_CLIENT_WAITLIST_STUDIO_SLUGS), and no other
// spec claims it: every other seeded studio gets a random
// `e2e-studio-<runId>` slug, so the rest of the browser suite runs with the
// feature OFF. That makes the extended run itself the flag-OFF regression
// proof — if this feature could leak into an unlisted studio, the existing
// public-booking specs would go red.
...
// e2e/helpers/local-env.ts), so `sendEmailSafely` genuinely attempts the send
// and the provider genuinely refuses it. The waitlist action is FAIL-CLOSED by
// design — no provider acceptance, no success — so the browser lane exercises
// the REFUSAL side of the commit law, which is the side that matters most
// here: it proves the UI never claims someone joined a waitlist that nobody
// received, and that nothing was written on the way there.
//
// The other side (provider ACCEPTS -> success UI) is proven in the unit lane,
// in tests/app/book/new-client-waitlist-action.test.ts, against a stubbed
// sender. It is NOT proven here on purpose: the only way to make this lane
// report success would be to give the action a code path that can succeed
// WITHOUT a real send, and such a path could also fire in production. The
// fail-closed guarantee is worth more than the assertion.
//
// Precedent for this split: e2e/practitioner-assisted-intake.spec.ts documents
// the same dummy-key reality and puts its load-bearing proof in the unit lane.
...
async function countsFor(studioId: string) {
  const [clients] = await sql<{ n: string }>(
    `select count(*)::text as n from public.clients where studio_id = $1`,
    [studioId],
  );
  const [appointments] = await sql<{ n: string }>(
    `select count(*)::text as n from public.appointments where studio_id = $1`,
    [studioId],
  );
  const [marketing] = await sql<{ n: string }>(
    `select count(*)::text as n from public.waitlist where lower(email) = lower($1)`,
    [CANARY_EMAIL],
  );
  return {
...
// --- Scenario 1 + 3: NEW client on a waitlisted studio, at 390px ------------
test.describe("new client at a waitlisted studio", () => {
  ...
  test("sees the waitlist immediately, submits it, and nothing is written", async ({ page }) => {
    ...
    //     visitor is told so and is NEVER shown the success state.
    await expect(
...
// --- Scenario 2: EXISTING client on the SAME waitlisted studio --------------
test("an existing client at a waitlisted studio keeps the normal booking path", async ({ page }) => {
  ...
  // The returning-client surface is exactly what it was before this feature:
  // the studio's client-portal hand-off.
  await expect(
    page.getByRole("heading", { name: new RegExp(`already a ${seed.studioName} client`, "i") }),
  ).toBeVisible({ timeout: 20_000 });
  await expect(
    page.getByRole("link", { name: /sign in to client portal/i }),
  ).toBeVisible();
