...
// landing-page waitlist and keys on the hashed IP / hashed email ALONE, with
// no studio or surface component. Sharing it would mean:
//
//   * a visitor who used the landing-page early-access form twice is refused
//     on a studio's booking waitlist for a day, and
//   * two waitlisted studios silently consume each other's budgets.
//
// Both silently DROP a real new-client lead, which is precisely the demand
// this release exists to capture. So the booking waitlist gets its own Redis
// prefixes and its own key space, scoped by the SERVER-RESOLVED studio id.
//
// FAIL-OPEN, DELIBERATELY CLASSIFIED (not inherited by habit). A limiter
// outage here means either (a) fail closed and drop genuine leads during the
// exact window the studio is trying to capture them, or (b) fail open and risk
// extra operational emails in the studio inbox. (a) destroys the release's
// purpose and is unrecoverable — the lead is simply gone. (b) is noisy,
// visible, and recoverable: the operator can clear the feature flag. Fail open,
// matching every other public limiter in this file.
//
...
export async function limitNewClientBookingWaitlist(args: {
  ...
  const ip = clientIpFromHeaders(args.headers);
  try {
    // Identifier is hashed FIRST, then scoped by studio, so one studio's
    // traffic can never consume another's budget.
