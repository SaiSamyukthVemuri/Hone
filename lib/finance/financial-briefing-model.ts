...
/** One appointment, reduced to the only field this slice reasons about. */
export type CensusRow = { readonly status: string };
...
export type CalendarCensus = {
  ...
  /** `confirmed` — on the calendar, not yet closed out either way. */
...
/**
 * PURE. Counts one period's appointments by status.
 *
 ...
 */
export function summarizeCalendar(rows: readonly CensusRow[]): CalendarCensus {
  ...
  return {
    ...
    stillToHappen: count("confirmed"),
    completed: count("completed"),
    cancelled: count("cancelled"),
    noShow: count("no_show"),
    partition: {
      closed: unrecognised.size === 0,
...
export function unreadableCalendar(cause: FinancialUnknownCause): CalendarCensus {
  ...
  return {
    ...
    cancelled: absent,
    noShow: absent,
    partition: { closed: false, unrecognisedStatuses: [] },
  };
}
