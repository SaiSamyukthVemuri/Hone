...
describe("0131 — repo migration-max tripwire", () => {
  ...
  it("advances the repo migration max to 0152 (Chloe manual-override booking: actual-overlap hard / buffer soft)", () => {
    ...
    const nums = files
      ...
      .sort();
    expect(nums[nums.length - 1]).toBe("0162"); // 0161 = service order RPC + widened calendar-colour CHECK
    ...
    expect(files.filter((f) => /^01(6[3-9]|[7-9]\d)_/.test(f))).toEqual([]); // 0162 present (intake review DB boundary); trip on 0163+
  });
});
