...
describe("0131 — repo migration-max tripwire", () => {
  it("advances the repo migration max to 0131 (0130 precedes; nothing 0132+)", () => {
    ...
    const nums = files
      ...
      .sort();
    expect(nums[nums.length - 1]).toBe("0133"); // 0133 = practitioner Move appointment atomic RPC (repo-only until hosted-applied this rollout)
    ...
    // Nothing 0134+ yet. Bump this tripwire consciously when adding migrations.
