...
describe("0157 — whole-session copy (repo migration-max tripwire)", () => {
  it("is present, 0156 precedes it, exactly one 0157, nothing 0159+ (repo max pin now lives in the 0158 test)", () => {
    expect(FILE).toMatch(/^0157_.*\.sql$/);
    const files = readdirSync(MIG_DIR);
    expect(files.some((f) => f.startsWith("0156_"))).toBe(true);
    // Collision guard: exactly ONE 0157 migration. The absolute repo-max pin
    // moved to the 0158 test (0158 = finalized structured-area containment).
