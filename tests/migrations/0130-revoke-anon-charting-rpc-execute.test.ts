...
describe("0130 — file + repo-max tripwire", () => {
  ...
  it("is present; the repo-max tripwire now lives in the 0131 test (0126-0129 precede)", () => {
    ...
    // practitioner-capacity foundation). 0130 is present; nothing above 0138
    // may exist yet.
    ...
    expect(FILES.filter((f) => /^01(6[2-9]|[7-9]\d)_/.test(f))).toEqual([]); // 0155 now present (card-change dedupe); trip on 0156+
  });
