...
// PR #179. Verify that the Stripe gate counts are unchanged after
// adding webhook reconciliation handlers. The webhook helpers must
// not introduce any new allowlisted Stripe SDK call site.
...
describe("check-stripe-gates: unchanged by PR #179", () => {
  ...
  it("STRIPE_ALLOW_LIVE_MODE=true still allowlists lib/stripe/server.ts only", () => {
    expect(GATE).toMatch(
