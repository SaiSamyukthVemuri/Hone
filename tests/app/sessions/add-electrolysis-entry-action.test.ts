...
//   * Gate 5 — the read-back is a SEPARATE query by the inserted row id (scoped to
//     the session); a post-insert failure returns a distinct persisted-but-
//     unverified result carrying the entryId and does NOT auto-retry.
//
// The mock supabase distinguishes the INSERT chain (insert → select("id") →
// single) from the SEPARATE read chain (select("observation_chips") → eq(id) →
// eq(session_id) → maybeSingle) and from assertSessionVisible's sessions read,
  ...
  return {
    from(table: string) {
      ...
      Object.assign(builder, {
        ...
        // Only the INSERT chain calls .single() (RETURNING the new id).
        single: async () => state.insertReturn,
        maybeSingle: async () => {
...
function fd(opts: {
  ...
  f.set("session_id", "s1");
  f.set("client_id", "c1");
  f.set("block_id", "b1"); // provided → ensureBlockForSession is skipped
...
describe("Gate 5 — atomicity / retry safety", () => {
  ...
  it("INSERT failure → not_persisted, NO read-back query, zero persisted", async () => {
    state.insertReturn = { data: null, error: { message: "insert boom" } };
    const res = await addElectrolysisEntryAction(fd({ chips: JSON.stringify([A]) }));
  ...
  it("read-back MISMATCH → unverified and does NOT auto-issue a second insert", async () => {
    ...
    const res = await addElectrolysisEntryAction(fd({ chips: JSON.stringify([A]) }));
    expect(res).toMatchObject({ ok: false, code: "unverified" });
    expect(state.insertCalls).toBe(1); // exactly one — never a blind retry inside the action
  });

  it("comments stay SEPARATE from the chip array in the insert payload", async () => {
    state.readReturn = { data: { observation_chips: [A] }, error: null };
