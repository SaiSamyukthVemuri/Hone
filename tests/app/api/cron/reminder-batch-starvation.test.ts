...
describe("the real cron route actually pages", () => {
  ...
  it("does not BYPASS the sender authority, and reads no env sender", () => {
    ...
    // Selection moved to the database (0199), so the ROUTE no longer resolves
    // at all — which is the point: it cannot hold a sender and cannot decide
    // that a send is permitted. The final authority lives in the send helper,
    // reached only through the public send functions.
    const c = code();
    ...
    // The candidate RPC must not hand back a sender identifier.
