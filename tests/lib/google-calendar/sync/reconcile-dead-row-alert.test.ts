...
describe("sweepCalendarDeadRowAlerts", () => {
  it("iterates studios with dead rows; a store failure never throws", async () => {
    createAdminClient.mockReturnValue(adminReturning([]));
    const store = { listStudiosWithDeadOutbox: async () => [{ studioId: "s1", deadCount: 1 }, { studioId: "s2", deadCount: 2 }] };
    expect(await sweepCalendarDeadRowAlerts(store)).toEqual({ studios: 2, alerted: 2 });
    const failing = { listStudiosWithDeadOutbox: async () => { throw new Error("x"); } };
    await expect(sweepCalendarDeadRowAlerts(failing)).resolves.toEqual({ studios: 0, alerted: 0 });
  });
});
