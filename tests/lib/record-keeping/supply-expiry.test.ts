...
describe("supplyExpiryState", () => {
  ...
  it("today itself is expiring (not yet expired)", () => {
    expect(supplyExpiryState("2026-07-02", TODAY)).toBe("expiring");
  });
...
describe("summarizeSupplyExpiry (banner counts)", () => {
  it("counts expired and expiring separately; ignores neutral/null", () => {
    const rows = [
    ...
    expect(summarizeSupplyExpiry(rows, TODAY)).toEqual({ expired: 2, expiring: 1 });
  });
