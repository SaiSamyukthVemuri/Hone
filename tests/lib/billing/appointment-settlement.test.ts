...
describe("the write path touches no Stripe surface", () => {
  ...
  it("the quoted amount uses the SAME resolver the card path uses", () => {
    expect(ACTIONS).toMatch(/resolveAuthoritativeSessionPaymentAmount/);
    // And a price it cannot resolve is stored as null, never as a zero.
