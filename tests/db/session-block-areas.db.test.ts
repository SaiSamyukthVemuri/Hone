...
//
// UPDATED BY 0158. Structured areas are the AUTHORITATIVE clinical area record,
...
// way left to exercise a raw INSERT — the member-facing posture (SELECT yes,
// direct DML no, finalized parents frozen for every role) is proven in
...
describe("create + read (member reads what the trusted path wrote)", () => {
  it("records multiple areas with different laterality under one block", async () => {
    ...
    // The member still SELECTs its own studio's rows (0158 keeps read access).
...
describe("tenant isolation + studio-derive", () => {
  ...
  it("studio B cannot INSERT an area onto studio A's block", async () => {
    // Post-0158 the denial lands at the privilege layer (42501) before RLS is
  ...
  it("…and for service_role, the one NON-OWNER role that still holds DML (0158)", async () => {
    // The derive trigger is SECURITY INVOKER and reads public.session_blocks, so
    // it behaves differently per role. After 0158 revoked browser DML, service_role
  ...
  it("a studio_id-only UPDATE is re-derived, not honoured (0158 widened the trigger)", async () => {
    const { blockId } = await seedSession(a);
...
describe("editable + cascade", () => {
  ...
  it("a member holds NO direct DML on the table (0158)", async () => {
    const { blockId } = await seedSession(a);
    const id = (await adminQuery(INS, [blockId, a.studioId, "Cheek", "left", 0])).rows[0]
      .id as string;
    await expect(
